...
from .physics import apply_autumn_cooling_adjustment
...
def run_profile_kalman_filter(
    ...
    for day_idx, date_value in enumerate(normalized_dates):
        ...
        if day_obs is None or day_obs.empty:
            ...
            obs_std = []
            obs_std_scale_extra = day_obs.get('obs_std_scale', pd.Series(1.0, index=day_obs.index))
            for source, obs_depth, obs_extra_scale in zip(
                day_obs['source'],
                day_obs['grid_depth_m'],
                obs_std_scale_extra,
            ):
                base_std = depth_dependent_obs_std(
                    source=source,
                    depth_m=obs_depth,
                    max_depth=max_depth,
                ...
                obs_std.append(base_std * float(np.clip(obs_extra_scale, 1.0, 4.0)))
