...
def compute_heatmap_physics_diagnostics(
    ...
    column_jump_all = np.nanmax(np.abs(np.diff(hat, axis=0)), axis=1)
    ...
    if np.any(april_mask):
        ...
        april_surface_jump = float(np.nanmax(surface_jump_all[april_jump_mask])) if np.any(april_jump_mask) else np.nan
    else:
    ...
    max_surface_jump_pos = int(np.nanargmax(surface_jump)) if surface_jump.size else -1
    max_column_jump_pos = int(np.nanargmax(column_jump)) if column_jump.size else -1
    ...
    return {
        ...
        "heatmap_max_weak_density_unstable_layer_frac": float(np.nanmax(weak_unstable_frac)) if weak_unstable_frac.size else np.nan,
        "heatmap_max_density_unstable_layer_frac": float(np.nanmax(strong_unstable_frac)) if strong_unstable_frac.size else np.nan,
...
def evaluate_vetoes(
    ...
    failed_checks = [
        label
        for label, passed in [
            ("winter_inverse", winter_inverse_pass),
