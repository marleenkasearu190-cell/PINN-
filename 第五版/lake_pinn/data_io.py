...
def pick_numeric_series_ci(frame: pd.DataFrame, candidates, default=np.nan):
    column = first_existing_column_ci(frame, candidates)
    if column is None:
        return pd.Series(default, index=frame.index, dtype=np.float32)
    values = pd.to_numeric(frame[column], errors='coerce')
    if values.isna().all():
...
def prepare_lst_daily_frame(lst: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    out = lst[['Date']].copy()
    if first_existing_column_ci(lst, ['LST_surface_C']) is not None:
        lst_c = pick_numeric_series_ci(lst, ['LST_surface_C'])
        out['LST_surface_K'] = lst_c + 273.15
    else:
        out['LST_surface_K'] = pick_numeric_series_ci(
            lst,
            ['LST_surface_K', 'MOD11A1_061_LST_Day_1km', 'lst_surface_k'],
        )
...
def finalize_lst_quality_columns(frame: pd.DataFrame, lst_missing_before_fill: pd.Series) -> pd.DataFrame:
    ...
    for column in ('LST_valid_pixel_fraction', 'LST_qc_good_fraction'):
        ...
        if frame[column].max(skipna=True) > 1.5:
            frame[column] = frame[column] / 100.0
        frame[column] = frame[column].fillna(1.0).clip(0.0, 1.0)
...
def compute_lst_quality_factor(frame: pd.DataFrame) -> pd.Series:
    ...
    if 'LST_valid_pixel_fraction' in frame.columns:
        valid_fraction = pd.to_numeric(frame['LST_valid_pixel_fraction'], errors='coerce').fillna(1.0)
        if valid_fraction.max(skipna=True) > 1.5:
            valid_fraction = valid_fraction / 100.0
        factor *= valid_fraction.clip(0.15, 1.0)

    if 'LST_qc_good_fraction' in frame.columns:
        qc_fraction = pd.to_numeric(frame['LST_qc_good_fraction'], errors='coerce').fillna(1.0)
        if qc_fraction.max(skipna=True) > 1.5:
            qc_fraction = qc_fraction / 100.0
...
def add_past_forcing_memory_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    def past_mean(column: str, window: int) -> pd.Series:
        return out[column].rolling(window=window, min_periods=1).mean()

    def past_sum(column: str, window: int) -> pd.Series:
        return out[column].rolling(window=window, min_periods=1).sum()
    ...
    heating_degree = (out['T_air_C'] - 4.0).clip(lower=0.0)
...
def load_training_frame(era5_path, lst_path):
    ...
    merged = add_past_forcing_memory_features(merged)
...
def build_observation_dataframe(
    ...
    if any(column in observations.columns for column in LST_QUALITY_COLUMNS):
