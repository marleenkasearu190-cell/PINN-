...
def infer_output_dir(era5_path: Path, lst_path: Path) -> Path:
    if era5_path.parent == lst_path.parent:
        return era5_path.parent / 'outputs'
    return era5_path.parent / 'outputs'
...
def load_training_frame(era5_path, lst_path):
    ...
    if merged['MixedLayerDepth_m'].isna().all():
        merged['MixedLayerDepth_m'] = 0.0
    else:
        merged['MixedLayerDepth_m'] = (
            merged['MixedLayerDepth_m']
            .interpolate(method='linear', limit_direction='both')
            .bfill()
            .ffill()
    ...
    if merged[required].isna().any().any():
        missing_counts = merged[required].isna().sum()
        missing_counts = missing_counts[missing_counts > 0]
        raise ValueError(
            'Input data still contains missing values after preprocessing: '
...
def main():
    ...
    parser.add_argument('--output-dir', default=None, help='Directory for saved outputs.')
    args = parser.parse_args()

    era5_path = prompt_for_existing_path('ERA5')
    lst_path = prompt_for_existing_path('LST')
    ...
    full_csv_path = export_temperature_tables(df, temp_grid, depths, output_dir, metadata)
