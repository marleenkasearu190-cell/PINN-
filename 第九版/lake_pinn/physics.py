...
def compute_surface_flux_terms(
    ...
    *,
    ...
    exchange_scale = torch.as_tensor(
        turbulent_exchange_scale,
        device=surface_temp.device,
        dtype=surface_temp.dtype,
    )
    if exchange_scale.ndim == 0:
        exchange_scale = torch.full_like(surface_temp, float(exchange_scale))
    elif exchange_scale.numel() == 1 and surface_temp.numel() > 1:
        exchange_scale = torch.full_like(surface_temp, float(exchange_scale.reshape(-1)[0]))
    else:
        exchange_scale = exchange_scale.reshape_as(surface_temp)
    exchange_scale = torch.clamp(exchange_scale, min=0.0)

    sensible_heat_bulk_unscaled = (
        air_density * AIR_HEAT_CAPACITY * TRANSFER_COEFF_HEAT * wind_speed * (surface_temp - air_temp)
    )
    latent_heat_bulk_unscaled = (
        air_density * LATENT_HEAT_VAPORIZATION * TRANSFER_COEFF_MOISTURE * wind_speed * (q_surface - q_air)
    )
    sensible_heat_bulk = exchange_scale * sensible_heat_bulk_unscaled
    latent_heat_bulk = exchange_scale * latent_heat_bulk_unscaled
    if mode == 'bulk':
        sensible_heat_provided = None
        latent_heat_provided = None
    else:
    ...
    blend_alpha = torch.as_tensor(
        turbulent_flux_blend_alpha,
        device=surface_temp.device,
        dtype=surface_temp.dtype,
    )
    if blend_alpha.ndim == 0:
        blend_alpha = torch.full_like(surface_temp, float(blend_alpha))
    elif blend_alpha.numel() == 1 and surface_temp.numel() > 1:
        blend_alpha = torch.full_like(surface_temp, float(blend_alpha.reshape(-1)[0]))
    else:
        blend_alpha = blend_alpha.reshape_as(surface_temp)
    blend_alpha = torch.clamp(blend_alpha, 0.0, 1.0)
    ...
    return {
        ...
        'sensible_heat_bulk_unscaled': sensible_heat_bulk_unscaled,
        'latent_heat_bulk_unscaled': latent_heat_bulk_unscaled,
        'turbulent_exchange_scale': exchange_scale,
