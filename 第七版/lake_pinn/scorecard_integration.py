"""Scorecard integration and prediction diagnostic plotting helpers."""
...
def _infer_year_label_from_dates(dates):
...
def _lake_year_label(lake_name, dates):
    lake_name = str(lake_name or 'Lake').strip()
    year_label = _infer_year_label_from_dates(dates)
...
def _plot_bias_contour_heatmap(matched, lake_name, model_label, output_path):
    _ensure_chinese_font()
    max_depth = float(matched['Depth_m'].max()) if not matched.empty else 1.0
    lake_year = _lake_year_label(lake_name, matched['Date'])
    ...
    ax.set_ylabel('Depth (m) | 深度', fontsize=16, fontweight='bold', labelpad=12)
    ax.set_title(
        f'Interpolated Bias Contour | {lake_year} | 插值误差等值图',
        fontsize=23,
        fontweight='bold',
        pad=16,
    )
    ax.text(
        0.0,
        1.01,
        f'{lake_year} | {model_label} | Bias = Prediction - Observation',
        transform=ax.transAxes,
        fontsize=13,
        color='#555555',
        ha='left',
