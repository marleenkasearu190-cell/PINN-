"""Forcing preprocessing and surface-temperature target helpers."""

import numpy as np
import pandas as pd

from .constants import SURFACE_AIR_BLEND, SURFACE_SKIN_COOLING_COEF
...
def suppress_spring_lst_warm_spikes(
    ...
    spring days. The PINN uses LST as an input feature, so those spikes need to
    be treated as unreliable skin excursions, not as a full mixed-layer warming
    event.
    """
    lst_surface_c = np.asarray(lst_surface_c, dtype=np.float64)
    air_temp_c = np.asarray(air_temp_c, dtype=np.float64)
    ...
    if str(data_fill_mode or 'reconstruction').strip().lower() != 'reconstruction':
        raise ValueError("Unsupported data_fill_mode; forecast fill mode was removed. Use reconstruction.")
    filled = (
