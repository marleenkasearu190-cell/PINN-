...
class LakeStateForecaster(nn.Module):
    ...
    def _adaptive_parameter_values(self, static_features, forcing_context=None, forcing_features=None):
        if static_features.ndim == 1:
            static_features = static_features.unsqueeze(0)
        batch_size = static_features.shape[0]
        ...
        if not active_modes:
            return base_values, torch.zeros_like(base_wind)
        ...
        if self.lake_adaptive_temporal_head is not None and self.lake_adaptive_temporal_scale > 0.0:
