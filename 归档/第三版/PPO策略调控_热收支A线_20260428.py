...
# PDF-aligned FLake/Hostetler-style diffusivity parameters
...
DIFFUSIVITY_EDDY_SCALE = 1.0
DIFFUSIVITY_UNSTABLE_BOOST = 1.0
DIFFUSIVITY_MAX_TOTAL = 2.5e-2
HEAT_BUDGET_RESIDUAL_SCALE_W_M2 = 150.0
...
PINN_INPUT_FORCING_SMOOTHING_WINDOW_DAYS = 3
KNOWN_LAKE_ATTRIBUTES = {
    'mohonk': {'area_km2': 0.30, 'latitude': 41.76},
    'mohonk lake': {'area_km2': 0.30, 'latitude': 41.76},
    'mendota': {'area_km2': 39.40, 'latitude': 43.10},
    'lake mendota': {'area_km2': 39.40, 'latitude': 43.10},
}
...
def apply_practical_prediction_defaults(args: argparse.Namespace, has_profile_obs: bool) -> argparse.Namespace:
    ...
    args.density_reg_weight = 0.10
    args.diffusivity_eddy_scale = 1.0
    args.diffusivity_unstable_boost = 1.0
    args.heat_budget_residual_scale_wm2 = 150.0
...
def apply_train_mode_defaults(args: argparse.Namespace, has_profile_obs: bool) -> argparse.Namespace:
    ...
    args.surface_mixed_layer_uniformity_weight = 0.14
    args.abrupt_surface_cooling_weight = 0.10
...
def apply_physics_skeleton_mode(args: argparse.Namespace) -> argparse.Namespace:
    args.use_ppo = False
    args.online_ppo_update = False
    args.ppo_tune_kalman = False
    args.ppo_use_kalman_reward = False
    args.time_continuity_weight = 0.0
    args.stratification_weight = 0.0
    args.smoothness_weight = 0.05
    args.deep_warming_weight = 0.02
    args.deep_anchor_weight = 0.0
    args.vertical_exchange_weight = 0.03
    args.convective_mixing_weight = 0.03
    args.surface_mixed_layer_uniformity_weight = 0.02
    args.abrupt_surface_cooling_weight = 0.02
    args.autumn_overturn_weight = 0.03
    args.heat_budget_weight = 0.03
    args.density_reg_weight = 0.01
    args.diffusivity_eddy_scale = 10.0
    args.diffusivity_unstable_boost = 100.0
    args.heat_budget_residual_scale_wm2 = 150.0
    return args
...
def scheduled_heat_budget_weight(
    epoch: int,
    target: float = 0.005,
    start_epoch: int = 150,
    end_epoch: int = 300,
    ...
    return target * float(progress)


def heat_budget_autumn_multiplier(
    doy: torch.Tensor,
    autumn_start: float = 250.0,
    autumn_end: float = 325.0,
    autumn_multiplier: float = 0.5,
    sharpness: float = 6.0,
) -> torch.Tensor:
    """Smoothly reduce heat-budget loss during autumn overturn transition."""
    doy = doy.float()
    enter = torch.sigmoid((doy - float(autumn_start)) / float(sharpness))
    leave = torch.sigmoid((float(autumn_end) - doy) / float(sharpness))
    autumn_gate = enter * leave
    return 1.0 - (1.0 - float(autumn_multiplier)) * autumn_gate
...
def load_training_frame(era5_path, lst_path):
    ...
    merged['PINN_input_wind_speed_m_per_s'] = smooth_series_for_pinn_input(merged['wind_speed_m_per_s'])
    merged['PINN_input_Solar_W_m2'] = smooth_series_for_pinn_input(merged['Solar_W_m2'])
    merged['PINN_input_LST_surface_C'] = smooth_series_for_pinn_input(merged['LST_surface_C'])
...
def smooth_series_for_pinn_input(series: pd.Series, window_days: int = PINN_INPUT_FORCING_SMOOTHING_WINDOW_DAYS) -> pd.Series:
    values = pd.to_numeric(series, errors='coerce').astype(float)
    return (
        values
        .rolling(window=max(int(window_days), 1), min_periods=1, center=True)
        .mean()
        .bfill()
        .ffill()
    )
...
def apply_forcing_adjustments(
    ...
    if solar_shading_factor != 1.0:
        ...
        if 'PINN_input_Solar_W_m2' in adjusted.columns:
            adjusted['PINN_input_Solar_W_m2'] = adjusted['PINN_input_Solar_W_m2'] * solar_shading_factor
...
def build_observation_dataframe(
    ...
    forcing_columns = [
        ...
        'Solar_W_m2',
        'LST_surface_C',
        'PINN_input_wind_speed_m_per_s',
        'PINN_input_Solar_W_m2',
        'PINN_input_LST_surface_C',
    ]
...
def build_model_inputs(
    ...
    if int(getattr(model, 'input_dim', 2)) <= 2:
...
def compute_diffusivity(
    ...
    shortwave=None,
    lst_surface=None,
    input_wind_speed=None,
    input_shortwave=None,
    input_lst_surface=None,
):
    temp_pred = model_temperature(
        ...
        doy_cos=doy_cos,
        air_temp=air_temp,
        wind_speed=input_wind_speed if input_wind_speed is not None else wind_speed,
        shortwave=input_shortwave if input_shortwave is not None else shortwave,
        lst_surface=input_lst_surface if input_lst_surface is not None else lst_surface,
    )
    ...
    ri_eff = richardson_number.clamp(min=-0.19, max=50.0)
    stability_factor = (1.0 + DIFFUSIVITY_RI_SENSITIVITY * ri_eff).clamp(min=0.05)
    wind_decay = torch.exp(-z_col / DIFFUSIVITY_WIND_DECAY_DEPTH)
    wind_mixing = (
        DIFFUSIVITY_WIND_COEFF
        * wind_speed.clamp(min=0.1) ** DIFFUSIVITY_WIND_EXPONENT
        * wind_decay
    )

    eddy_diffusivity_base = (
        DIFFUSIVITY_EDDY_SCALE
        * DIFFUSIVITY_K0
        * wind_mixing
        * stability_factor.pow(-DIFFUSIVITY_ALPHA)
    )
    total_diffusivity = MOLECULAR_DIFFUSIVITY + eddy_diffusivity_base

    unstable_boost = torch.where(
        density_gradient < 0.0,
        torch.full_like(total_diffusivity, DIFFUSIVITY_UNSTABLE_BOOST),
        torch.ones_like(total_diffusivity),
    )

    diffusivity = (total_diffusivity * unstable_boost).clamp(
        min=MIN_TOTAL_DIFFUSIVITY,
        max=DIFFUSIVITY_MAX_TOTAL,
    )
...
def compute_losses(
    ...
    temp_pred, dT_dt_norm, dT_dz, density_gradient, richardson_number, diffusivity = compute_diffusivity(
        ...
        input_wind_speed=batch.get('model_wind_colloc'),
        input_shortwave=batch.get('model_solar_flux_colloc'),
        input_lst_surface=batch.get('model_lst_surface_colloc'),
    )
    ...
    surface_temp = model_temperature(
        ...
        wind_speed=batch.get('model_surface_wind_speed'),
        shortwave=batch.get('model_surface_shortwave'),
        lst_surface=batch.get('model_surface_lst'),
    )
    ...
    _, _, _, _, _, diffusivity_surface = compute_diffusivity(
        ...
        input_wind_speed=batch.get('model_surface_wind_speed'),
        input_shortwave=batch.get('model_surface_shortwave'),
        input_lst_surface=batch.get('model_surface_lst'),
    )
    ...
    ic_temp_pred = model_temperature(
        ...
        wind_speed=batch.get('model_ic_wind_speed'),
        shortwave=batch.get('model_ic_shortwave'),
        lst_surface=batch.get('model_ic_lst_surface'),
    )
    ...
    obs_temp_pred = model_temperature(
        ...
        wind_speed=batch.get('model_obs_wind_speed'),
        shortwave=batch.get('model_obs_shortwave'),
        lst_surface=batch.get('model_obs_lst_surface'),
    )
    ...
    if batch.get('seq_time_now') is not None and batch['seq_time_now'].numel() > 0:
        ...
        temp_seq_now, _, dT_dz_seq, _, _, diffusivity_seq = compute_diffusivity(
            ...
            input_wind_speed=batch.get('model_seq_wind_now'),
            input_shortwave=batch.get('model_seq_solar_flux_now'),
            input_lst_surface=batch.get('model_seq_lst_now'),
        )
        ...
        temp_seq_next = model_temperature(
            ...
            wind_speed=batch.get('model_seq_wind_next'),
            shortwave=batch.get('model_seq_solar_flux_next'),
            lst_surface=batch.get('model_seq_lst_next'),
        )
    ...
    if batch.get('seq_time_now') is not None and batch['seq_time_now'].numel() > 0:
        ...
        surface_seq_now = model_temperature(
            ...
            doy_cos=batch.get('seq_doy_cos_now'),
            air_temp=batch.get('seq_air_temp_now'),
            wind_speed=batch.get('model_seq_wind_now'),
            shortwave=batch.get('model_seq_solar_flux_now'),
            lst_surface=batch.get('model_seq_lst_now'),
        )
    ...
    if batch.get('strat_time') is not None and batch['strat_time'].numel() > 0:
        ...
        shallow_temp = model_temperature(
            ...
            wind_speed=batch.get('model_strat_wind_speed'),
            shortwave=batch.get('model_strat_shortwave'),
            lst_surface=batch.get('model_strat_lst_surface'),
        )
    ...
    if batch.get('deep_anchor_time') is not None and batch['deep_anchor_time'].numel() > 0:
        deep_temp = model_temperature(
            ...
            wind_speed=batch.get('model_deep_anchor_wind_speed'),
            shortwave=batch.get('model_deep_anchor_shortwave'),
            lst_surface=batch.get('model_deep_anchor_lst_surface'),
        )
    ...
    heat_budget_residual_rms_autumn = torch.full((), float('nan'), dtype=torch.float32, device=t_col.device)
    heat_budget_weighted_all = torch.zeros((), dtype=torch.float32, device=t_col.device)
    heat_budget_weighted_autumn = torch.full((), float('nan'), dtype=torch.float32, device=t_col.device)
    if batch.get('budget_time_now') is not None and batch['budget_time_now'].numel() > 0:
        ...
        temp_budget_now = model_temperature(
            ...
            wind_speed=batch.get('model_budget_wind_speed_now_expanded'),
            shortwave=batch.get('model_budget_solar_flux'),
            lst_surface=batch.get('model_budget_lst_surface_now_expanded'),
        )
        temp_budget_next = model_temperature(
            ...
            wind_speed=batch.get('model_budget_wind_speed_next_expanded'),
            shortwave=batch.get('model_budget_solar_flux_next'),
            lst_surface=batch.get('model_budget_lst_surface_next_expanded'),
        )
        ...
        integrated_heat_tendency = torch.zeros_like(batch['budget_surface_flux_wm2'])
        ...
        internal_heating_flux = (-dphi_dz_budget) * dz_budget
        integrated_internal_heating = torch.zeros_like(batch['budget_surface_flux_wm2'])
        integrated_internal_heating.scatter_add_(0, budget_group_index, internal_heating_flux)

        external_heat_tendency = batch['budget_surface_flux_wm2'] + integrated_internal_heating
        heat_budget_residual = integrated_heat_tendency - external_heat_tendency
        heat_budget_residual_norm = heat_budget_residual / max(HEAT_BUDGET_RESIDUAL_SCALE_W_M2, 1.0)
        budget_doy_avg = 0.5 * (batch['budget_doy_now'] + batch['budget_doy_next'])
        heat_loss_multiplier = heat_budget_autumn_multiplier(
            budget_doy_avg,
            autumn_start=250.0,
            autumn_end=325.0,
            autumn_multiplier=0.5,
            sharpness=6.0,
        )
        heat_budget_sq = heat_budget_residual_norm ** 2
        loss_heat_budget = torch.mean(heat_loss_multiplier * heat_budget_sq)
        heat_budget_residual_rms = torch.sqrt(torch.mean(heat_budget_residual ** 2) + 1e-12)
        heat_budget_weighted_all = weights.get('heat_budget', 0.0) * torch.mean(heat_loss_multiplier * heat_budget_sq)
        is_autumn = (budget_doy_avg >= 250.0) & (budget_doy_avg <= 325.0)
        if torch.any(is_autumn):
            heat_budget_residual_rms_autumn = torch.sqrt(torch.mean(heat_budget_residual[is_autumn] ** 2) + 1e-12)
            heat_budget_weighted_autumn = (
                weights.get('heat_budget', 0.0)
                * torch.mean(heat_loss_multiplier[is_autumn] * heat_budget_sq[is_autumn])
            )
    ...
    return {
        ...
        'heat_budget_residual_rms_autumn': heat_budget_residual_rms_autumn,
        'heat_budget_weighted_all': heat_budget_weighted_all.detach(),
        'heat_budget_weighted_autumn': heat_budget_weighted_autumn.detach(),
...
def train_model(
    ...
    heat_budget_schedule_target_weight=None,
    heat_budget_schedule_start_epoch=150,
    heat_budget_schedule_end_epoch=300,
    ...
    model_input_dim = int(resume_input_dim if resume_input_dim is not None else PINN_INPUT_DIM)
    ...
    if heat_budget_schedule_target_weight is not None:
        weights['heat_budget'] = float(heat_budget_schedule_target_weight)
    ...
    model_solar_flux = torch.tensor(
        df.get('PINN_input_Solar_W_m2', df['Solar_W_m2']).to_numpy(dtype=np.float32).reshape(-1, 1),
        dtype=torch.float32,
        device=device,
    )
    model_wind_speed = torch.tensor(
        df.get('PINN_input_wind_speed_m_per_s', df['wind_speed_m_per_s']).to_numpy(dtype=np.float32).reshape(-1, 1),
        dtype=torch.float32,
        device=device,
    )
    model_lst_surface = torch.tensor(
        df.get('PINN_input_LST_surface_C', df['LST_surface_C']).to_numpy(dtype=np.float32).reshape(-1, 1),
        dtype=torch.float32,
        device=device,
    )
    ...
    model_obs_wind_speed = torch.tensor(
        observations.get('PINN_input_wind_speed_m_per_s', observations['wind_speed_m_per_s']).to_numpy(dtype=np.float32).reshape(-1, 1),
        device=device,
    )
    model_obs_shortwave = torch.tensor(
        observations.get('PINN_input_Solar_W_m2', observations['Solar_W_m2']).to_numpy(dtype=np.float32).reshape(-1, 1),
        device=device,
    )
    model_obs_lst_surface = torch.tensor(
        observations.get('PINN_input_LST_surface_C', observations['LST_surface_C']).to_numpy(dtype=np.float32).reshape(-1, 1),
        device=device,
    )
    ...
    model_ic_wind_speed = torch.full_like(ic_time, float(df.get('PINN_input_wind_speed_m_per_s', df['wind_speed_m_per_s']).iloc[0]))
    model_ic_shortwave = torch.full_like(ic_time, float(df.get('PINN_input_Solar_W_m2', df['Solar_W_m2']).iloc[0]))
    model_ic_lst_surface = torch.full_like(ic_time, float(df.get('PINN_input_LST_surface_C', df['LST_surface_C']).iloc[0]))
    ...
    for epoch in range(epochs):
        ...
        if heat_budget_schedule_target_weight is not None:
            ...
            effective_weights['heat_budget'] = scheduled_heat_budget_weight(
                epoch,
                target=heat_budget_schedule_target_weight,
                start_epoch=heat_budget_schedule_start_epoch,
                end_epoch=heat_budget_schedule_end_epoch,
            )
        ...
        if n_days > 1:
            ...
            model_seq_solar_flux_now = model_solar_flux[seq_pick]
            model_seq_solar_flux_next = model_solar_flux[seq_pick + 1]
            seq_wind_now = wind_speed[seq_pick]
            seq_wind_next = wind_speed[seq_pick + 1]
            model_seq_wind_now = model_wind_speed[seq_pick]
            model_seq_wind_next = model_wind_speed[seq_pick + 1]
            ...
            model_seq_lst_now = model_lst_surface[seq_pick]
            model_seq_lst_next = model_lst_surface[seq_pick + 1]
            ...
            model_seq_solar_flux_now = None
            model_seq_solar_flux_next = None
            seq_wind_now = None
            seq_wind_next = None
            model_seq_wind_now = None
            model_seq_wind_next = None
            seq_air_temp_now = None
            seq_air_temp_next = None
            seq_lst_now = None
            seq_lst_next = None
            model_seq_lst_now = None
            model_seq_lst_next = None
            seq_doy_now = None
            seq_mld_now = None
            seq_mld_next = None
            seq_deep_floor = None
        ...
        model_strat_wind_speed = model_wind_speed[strat_pick]
        model_strat_solar = model_solar_flux[strat_pick]
        model_strat_lst_surface = model_lst_surface[strat_pick]
        ...
        model_deep_wind_speed = model_wind_speed[deep_pick]
        model_deep_solar = model_solar_flux[deep_pick]
        model_deep_lst_surface = model_lst_surface[deep_pick]
        ...
        if n_days > 1:
            ...
            budget_doy_now = torch.tensor(
                df['full_doy'].iloc[budget_pick.detach().cpu().numpy()].to_numpy(dtype=np.float32).reshape(-1, 1),
                dtype=torch.float32,
                device=device,
            )
            budget_doy_next = torch.tensor(
                df['full_doy'].iloc[(budget_pick + 1).detach().cpu().numpy()].to_numpy(dtype=np.float32).reshape(-1, 1),
                dtype=torch.float32,
                device=device,
            )
            ...
            model_budget_solar_flux = model_solar_flux[budget_pick].repeat_interleave(depth_line.shape[0], dim=0)
            model_budget_solar_flux_next = model_solar_flux[budget_pick + 1].repeat_interleave(depth_line.shape[0], dim=0)
            ...
            model_budget_wind_speed = model_wind_speed[budget_pick]
            model_budget_wind_speed_next = model_wind_speed[budget_pick + 1]
            model_budget_lst_surface = model_lst_surface[budget_pick]
            model_budget_lst_surface_next = model_lst_surface[budget_pick + 1]
            ...
            model_budget_wind_speed_now_expanded = model_budget_wind_speed.repeat_interleave(depth_line.shape[0], dim=0)
            model_budget_wind_speed_next_expanded = model_budget_wind_speed_next.repeat_interleave(depth_line.shape[0], dim=0)
            model_budget_lst_surface_now_expanded = model_budget_lst_surface.repeat_interleave(depth_line.shape[0], dim=0)
            model_budget_lst_surface_next_expanded = model_budget_lst_surface_next.repeat_interleave(depth_line.shape[0], dim=0)
        else:
            budget_depth = None
            budget_doy_now = None
            budget_doy_next = None
            ...
            model_budget_solar_flux = None
            model_budget_solar_flux_next = None
            ...
            model_budget_wind_speed = None
            model_budget_wind_speed_next = None
            model_budget_lst_surface = None
            model_budget_lst_surface_next = None
            ...
            model_budget_wind_speed_now_expanded = None
            model_budget_wind_speed_next_expanded = None
            model_budget_lst_surface_now_expanded = None
            model_budget_lst_surface_next_expanded = None
        ...
        batch = {
            ...
            'model_solar_flux_colloc': model_solar_flux[day_pick],
            'model_wind_colloc': model_wind_speed[day_pick],
            'model_lst_surface_colloc': model_lst_surface[day_pick],
            'surface_time': t_all,
            ...
            'model_surface_shortwave': model_solar_flux,
            'model_surface_wind_speed': model_wind_speed,
            'model_surface_lst': model_lst_surface,
            ...
            'model_ic_wind_speed': model_ic_wind_speed,
            'model_ic_shortwave': model_ic_shortwave,
            'model_ic_lst_surface': model_ic_lst_surface,
            'obs_time': obs_time,
            'obs_depth': obs_depth,
            ...
            'model_obs_wind_speed': model_obs_wind_speed,
            'model_obs_shortwave': model_obs_shortwave,
            'model_obs_lst_surface': model_obs_lst_surface,
            ...
            'model_seq_solar_flux_now': model_seq_solar_flux_now,
            'model_seq_solar_flux_next': model_seq_solar_flux_next,
            'model_seq_wind_now': model_seq_wind_now,
            'model_seq_wind_next': model_seq_wind_next,
            'model_seq_lst_now': model_seq_lst_now,
            'model_seq_lst_next': model_seq_lst_next,
            ...
            'model_strat_wind_speed': model_strat_wind_speed,
            'model_strat_shortwave': model_strat_solar,
            'model_strat_lst_surface': model_strat_lst_surface,
            ...
            'model_deep_anchor_wind_speed': model_deep_wind_speed,
            'model_deep_anchor_shortwave': model_deep_solar,
            'model_deep_anchor_lst_surface': model_deep_lst_surface,
            'deep_anchor_depth': deep_depth,
            'deep_anchor_target': deep_target,
            'deep_anchor_weight': deep_weight,
            'budget_depth': budget_depth,
            'budget_doy_now': budget_doy_now,
            'budget_doy_next': budget_doy_next,
            ...
            'model_budget_solar_flux': model_budget_solar_flux,
            'model_budget_solar_flux_next': model_budget_solar_flux_next,
            ...
            'model_budget_wind_speed': model_budget_wind_speed,
            'model_budget_wind_speed_next': model_budget_wind_speed_next,
            'model_budget_wind_speed_now_expanded': model_budget_wind_speed_now_expanded,
            'model_budget_wind_speed_next_expanded': model_budget_wind_speed_next_expanded,
            'model_budget_lst_surface': model_budget_lst_surface,
            'model_budget_lst_surface_next': model_budget_lst_surface_next,
            'model_budget_lst_surface_now_expanded': model_budget_lst_surface_now_expanded,
            'model_budget_lst_surface_next_expanded': model_budget_lst_surface_next_expanded,
        ...
        if budget_surface_time is not None:
            budget_surface_temp_now = model_temperature(
                model,
                budget_surface_time,
                ...
                wind_speed=model_budget_wind_speed,
                shortwave=model_solar_flux[budget_pick],
                lst_surface=model_budget_lst_surface,
            )
            budget_flux_terms_now = compute_surface_flux_terms(
                budget_surface_temp_now,
                {
            ...
            budget_surface_temp_next = model_temperature(
                model,
                budget_time_next[::depth_line.shape[0]],
                budget_surface_depth,
                max_depth,
                metadata=metadata,
                doy_sin=doy_sin_all[budget_pick + 1],
                doy_cos=doy_cos_all[budget_pick + 1],
                air_temp=budget_air_temp_next,
                wind_speed=model_budget_wind_speed_next,
                shortwave=model_solar_flux[budget_pick + 1],
                lst_surface=model_budget_lst_surface_next,
            )
            budget_flux_terms_next = compute_surface_flux_terms(
                budget_surface_temp_next,
                {
                    'surface_air_temp': budget_air_temp_next,
                    'surface_wind_speed': budget_wind_speed_next,
                    'surface_relative_humidity': budget_relative_humidity,
                    'surface_pressure': budget_surface_pressure,
                    'surface_shortwave': solar_flux[budget_pick + 1],
                },
                shortwave_surface_fraction=shortwave_surface_fraction,
            )
            surface_flux_now_wm2 = (
                budget_flux_terms_now['net_radiation']
                - budget_flux_terms_now['sensible_heat']
                - budget_flux_terms_now['latent_heat']
            )
            surface_flux_next_wm2 = (
                budget_flux_terms_next['net_radiation']
                - budget_flux_terms_next['sensible_heat']
                - budget_flux_terms_next['latent_heat']
            )
            batch['budget_surface_flux'] = budget_flux_terms_now['seb_flux']
            batch['budget_surface_flux_wm2_now'] = surface_flux_now_wm2
            batch['budget_surface_flux_wm2_next'] = surface_flux_next_wm2
            batch['budget_surface_flux_wm2'] = 0.5 * (
                surface_flux_now_wm2 + surface_flux_next_wm2
            )
        else:
            batch['budget_surface_flux'] = None
            batch['budget_surface_flux_wm2'] = None
            batch['budget_surface_flux_wm2_now'] = None
            batch['budget_surface_flux_wm2_next'] = None
        ...
        if epoch % 200 == 0 or epoch == epochs - 1:
            heat_weighted = effective_weights['heat_budget'] * losses['loss_heat_budget'].item()
            heat_weighted_autumn = losses['heat_budget_weighted_autumn'].item()
            heat_rms_autumn = losses['heat_budget_residual_rms_autumn'].item()
            print(
                ...
                f"adv={losses['loss_vertical_exchange'].item():.4f} | conv={losses['loss_convective_mixing'].item():.4f} | mix={losses['loss_surface_mixed_layer_uniformity'].item():.4f} | drop={losses['loss_abrupt_surface_cooling'].item():.4f} | overturn={losses['loss_autumn_overturn'].item():.4f} | heat={losses['loss_heat_budget'].item():.4f} | heat_wtd={heat_weighted:.4f} | heat_rms={losses['heat_budget_residual_rms'].item():.4f} | heat_wtd_aut={heat_weighted_autumn:.4f} | heat_rms_aut={heat_rms_autumn:.4f} | "
                ...
                f"adv_w={effective_weights['vertical_exchange']:.3f} | conv_w={effective_weights['convective_mixing']:.3f} | mix_w={effective_weights['surface_mixed_layer_uniformity']:.3f} | drop_w={effective_weights['abrupt_surface_cooling']:.3f} | overturn_w={effective_weights['autumn_overturn']:.3f} | heat_w={effective_weights['heat_budget']:.3f}"
            )
...
def run_seasonal_segmented_pipeline(
    ...
    density_reg_weight,
    heat_budget_schedule_target_weight,
    heat_budget_schedule_start_epoch,
    heat_budget_schedule_end_epoch,
    train_until_best,
    train_min_epochs,
    ...
    for segment in segments:
        ...
        model, training_info = train_model(
            ...
            heat_budget_schedule_target_weight=heat_budget_schedule_target_weight,
            heat_budget_schedule_start_epoch=heat_budget_schedule_start_epoch,
            heat_budget_schedule_end_epoch=heat_budget_schedule_end_epoch,
        ...
        for day_idx, time_point in enumerate(times):
            ...
            pred = model(
                build_model_inputs(
                    ...
                    wind_speed=float(df['PINN_input_wind_speed_m_per_s'].iloc[day_idx]) if 'PINN_input_wind_speed_m_per_s' in df.columns else (float(df['wind_speed_m_per_s'].iloc[day_idx]) if 'wind_speed_m_per_s' in df.columns else None),
                    shortwave=float(df['PINN_input_Solar_W_m2'].iloc[day_idx]) if 'PINN_input_Solar_W_m2' in df.columns else (float(df['Solar_W_m2'].iloc[day_idx]) if 'Solar_W_m2' in df.columns else None),
                    lst_surface=float(df['PINN_input_LST_surface_C'].iloc[day_idx]) if 'PINN_input_LST_surface_C' in df.columns else (float(df['LST_surface_C'].iloc[day_idx]) if 'LST_surface_C' in df.columns else None),
                )
            ).cpu().numpy().flatten()
...
def main():
    global DIFFUSIVITY_EDDY_SCALE, DIFFUSIVITY_UNSTABLE_BOOST, HEAT_BUDGET_RESIDUAL_SCALE_W_M2
    ...
    parser.add_argument('--physics-skeleton-mode', action='store_true', help='Run a bone-structure experiment: disable PPO and most hand-crafted seasonal penalties, keep PDE/SEB/obs plus weak smoothness and heat-budget closure')
    ...
    parser.add_argument('--density-reg-weight', type=float, default=0.10, help='Weight for penalizing density inversions directly as a fallback regularizer')
    parser.add_argument('--heat-budget-schedule-target-weight', type=float, default=None, help='Optional delayed-ramp target weight for the heat-budget loss; when set, this overrides PPO/default heat_budget during optimization')
    parser.add_argument('--heat-budget-schedule-start-epoch', type=int, default=150, help='Epoch before delayed heat-budget loss starts ramping up')
    parser.add_argument('--heat-budget-schedule-end-epoch', type=int, default=300, help='Epoch when delayed heat-budget loss reaches its target weight')
    ...
    if args.physics_skeleton_mode:
        args = apply_physics_skeleton_mode(args)

    DIFFUSIVITY_EDDY_SCALE = float(getattr(args, 'diffusivity_eddy_scale', DIFFUSIVITY_EDDY_SCALE))
    DIFFUSIVITY_UNSTABLE_BOOST = float(getattr(args, 'diffusivity_unstable_boost', DIFFUSIVITY_UNSTABLE_BOOST))
    HEAT_BUDGET_RESIDUAL_SCALE_W_M2 = float(
        max(getattr(args, 'heat_budget_residual_scale_wm2', HEAT_BUDGET_RESIDUAL_SCALE_W_M2), 1.0)
    )
    ...
    print("Physics skeleton mode enabled:" if args.physics_skeleton_mode else "Physics skeleton mode disabled:", bool(args.physics_skeleton_mode))
    ...
    print(
        "Wind-mixing Keddy: "
        f"scale={DIFFUSIVITY_EDDY_SCALE:.1f}*K0, "
        f"U^{DIFFUSIVITY_WIND_EXPONENT:.2f}*exp(-z/{DIFFUSIVITY_WIND_DECAY_DEPTH:.1f}), "
        f"inversion x{DIFFUSIVITY_UNSTABLE_BOOST:.0f}"
    )
    print(f"Heat-budget residual scale: {HEAT_BUDGET_RESIDUAL_SCALE_W_M2:.1f} W/m^2")
    ...
    print(f"Density regularization weight: {args.density_reg_weight:.3f}")
    ...
    if args.mode == 'predict':
        ...
        heat_budget_schedule_target_weight=args.heat_budget_schedule_target_weight,
        heat_budget_schedule_start_epoch=args.heat_budget_schedule_start_epoch,
        heat_budget_schedule_end_epoch=args.heat_budget_schedule_end_epoch,
