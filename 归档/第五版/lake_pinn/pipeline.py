...
from .scorecard_integration import score_prediction_candidates
...
def main():
    ...
    parser.add_argument('--profile-split-mode', choices=['none', 'depth_interleaved', 'time_blocked', 'seasonal_blocked'], default='seasonal_blocked', help='How to split profile observations into train/val/assim/test roles; seasonal_blocked holds out each season')
    ...
    parser.add_argument('--model-input-dim', type=int, default=PINN_INPUT_DIM, help='PINN input dimension: default 27 for extended forcing, past-only weather memory, and causal previous-state memory; use 24/17/11 only for archived-compatible retraining')
    ...
    parser.add_argument('--rolling-prediction-mode', action='store_true', help='Legacy diagnostic only: enable rolling forecast correction after raw PINN prediction')
    parser.add_argument('--disable-rolling-prediction', action='store_true', help='Deprecated no-op for the main line: raw PINN grid output is now the default')
    ...
    parser.add_argument('--profile-grid-physics-weight', type=float, default=0.0, help='Weight for training-time whole-profile heatmap physics constraints: day jumps, density stability, vertical gradients, and deep slow changes')
    parser.add_argument('--profile-grid-day-pairs', type=int, default=12, help='Number of adjacent day pairs sampled per epoch for whole-profile physics constraints')
    parser.add_argument('--profile-grid-depth-points', type=int, default=41, help='Depth grid points per sampled day for whole-profile physics constraints')
    parser.add_argument('--density-reg-weight', type=float, default=0.1, help='Weight for penalizing statically unstable density inversions during training')
    ...
    if args.mode == 'predict':
        ...
        if args.disable_rolling_prediction:
            args.rolling_prediction_mode = False
    ...
    print(f"Rolling prediction mode: {'enabled' if args.rolling_prediction_mode else 'disabled'}")
