...
def apply_practical_prediction_defaults(args: argparse.Namespace, has_profile_obs: bool) -> argparse.Namespace:
    ...
    args.rolling_memory_blend = 0.0
    args.rolling_surface_relaxation = 0.0
...
def apply_train_like_prediction_defaults(args: argparse.Namespace) -> argparse.Namespace:
    """Use train-mode rolling/forcing defaults while staying in predict mode.

    This diagnostic profile checks whether practical-predict defaults, rather
    than the checkpoint itself, are damaging seasonal structure.
    """
...
def configure_interactive_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    print('  rolling_prediction_mode=False | raw PINN output is the default')
