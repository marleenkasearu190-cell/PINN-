...
def fill_standard_series(series: pd.Series, mode: str, default=np.nan) -> pd.Series:
    mode = str(mode or "forecast").lower()
    values = pd.to_numeric(series, errors="coerce")
    if mode == "forecast":
        values = values.ffill().fillna(default)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-fill-mode", choices=["forecast", "reconstruction"], default="forecast")
