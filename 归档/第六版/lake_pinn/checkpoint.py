...
def export_model_checkpoint_bundle(model, training_info):
    ...
    model_class = str(getattr(model, 'model_class', model.__class__.__name__))
    return {
        ...
        'adapter_dim': int(getattr(model, 'adapter_dim', 0) or 0),
        'lake_attr_indices': list(getattr(model, 'lake_attr_indices', []) or []),
        'adapter_residual_limit_c': getattr(model, 'adapter_residual_limit_c', None),
        ...
        'training_info': {
            ...
            'final_loss_contributions': info.get('final_loss_contributions'),
            'loss_component_units': info.get('loss_component_units'),
...
def load_model_checkpoint_bundle(checkpoint_path, device='cpu'):
    ...
    if first_weight is None:
        first_weight = model_state.get('shared_trunk.0.weight')
    inferred_input_dim = int(bundle.get('input_dim', first_weight.shape[1]))
    hidden_dim = int(bundle.get('hidden_dim', 128))
    hidden_layers = int(bundle.get('hidden_layers', 8))
    model_class = str(bundle.get('model_class', 'LakePINN'))
    architecture = 'global_adapter' if model_class == 'GlobalAdaptiveLakePINN' else 'legacy'
    model = create_lake_model(
        model_architecture=architecture,
        input_dim=inferred_input_dim,
        hidden_dim=hidden_dim,
        hidden_layers=hidden_layers,
        adapter_dim=int(bundle.get('adapter_dim', 32) or 32),
        lake_attr_indices=bundle.get('lake_attr_indices'),
        adapter_residual_limit_c=bundle.get('adapter_residual_limit_c'),
    ).to(device)
