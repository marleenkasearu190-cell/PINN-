...
def suppress_spring_lst_warm_spikes(
    ...
    if str(data_fill_mode).lower() == 'forecast':
        filled = pd.Series(lst_surface_c).ffill().fillna(pd.Series(air_temp_c)).fillna(0.0).to_numpy(dtype=np.float64)
        air_filled = pd.Series(air_temp_c).ffill().fillna(pd.Series(lst_surface_c)).fillna(0.0).to_numpy(dtype=np.float64)
    else:
        filled = (
