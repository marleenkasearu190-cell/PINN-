...
from .model import create_lake_model, model_temperature
...
def build_causal_previous_state_memory(df, train_profile_obs, max_depth):
    ...
    bottom_series = df['BottomTemp_C'] if 'BottomTemp_C' in df.columns else None
    if bottom_series is not None and np.isfinite(pd.to_numeric(bottom_series, errors='coerce').iloc[0]):
        last_deep = float(pd.to_numeric(bottom_series, errors='coerce').interpolate(limit_direction='both').bfill().ffill().iloc[0])
    else:
...
LOSS_COMPONENT_MAP = (
    ...
    ('ic', 'loss_ic'),
    ('obs', 'loss_obs'),
    ('time_continuity', 'loss_time_continuity'),
    ('stratification', 'loss_stratification'),
    ('smoothness', 'loss_smoothness'),
    ('deep_warming', 'loss_deep_warming'),
    ('deep_anchor', 'loss_deep_anchor'),
    ('vertical_exchange', 'loss_vertical_exchange'),
    ('convective_mixing', 'loss_convective_mixing'),
    ('surface_mixed_layer_uniformity', 'loss_surface_mixed_layer_uniformity'),
    ('abrupt_surface_cooling', 'loss_abrupt_surface_cooling'),
    ('bottom_slow_change', 'loss_bottom_slow_change'),
    ('autumn_overturn', 'loss_autumn_overturn'),
    ('heat_budget', 'loss_heat_budget'),
    ('profile_grid_physics', 'loss_profile_grid_physics'),
    ('density_reg', 'loss_density_reg'),
)


LOSS_COMPONENT_UNITS = {
    'pde': 'native PDE residual MSE',
    'bc': 'surface/bottom boundary residual MSE',
    'ic': 'degC^2',
    'obs': 'weighted degC^2',
    'time_continuity': 'degC^2/day-step',
    'stratification': 'degC^2 one-sided margin',
    'smoothness': '(degC/m)^2 excess',
    'deep_warming': 'degC^2/day-step excess',
    'deep_anchor': 'degC^2',
    'vertical_exchange': 'native exchange residual MSE',
    'convective_mixing': 'native mixing residual MSE',
    'surface_mixed_layer_uniformity': 'degC^2',
    'abrupt_surface_cooling': 'degC^2/day-step excess',
    'bottom_slow_change': 'degC^2/day-step excess',
    'autumn_overturn': 'degC^2',
    'heat_budget': 'integrated heat-tendency residual MSE',
    'profile_grid_physics': 'composite grid physics loss',
    'density_reg': '(kg/m3)^2 excess',
}


def build_loss_contribution_row(epoch, losses, weights):
    row = {
        'epoch': int(epoch),
        'total': float(losses['total'].detach().cpu().item()),
    }
    weighted_sum = 0.0
    for component, loss_key in LOSS_COMPONENT_MAP:
        raw_value = float(losses[loss_key].detach().cpu().item())
        weight_value = float(weights.get(component, 0.0))
        weighted_value = raw_value * weight_value
        row[f'{component}_raw'] = raw_value
        row[f'{component}_weight'] = weight_value
        row[f'{component}_weighted'] = weighted_value
        weighted_sum += weighted_value
    row['weighted_sum_components'] = float(weighted_sum)
    return row


def format_top_loss_contributors(row, top_n=4):
    contributors = []
    for component, _ in LOSS_COMPONENT_MAP:
        contributors.append((component, float(row.get(f'{component}_weighted', 0.0))))
    contributors = sorted(contributors, key=lambda item: abs(item[1]), reverse=True)[:top_n]
    return ','.join(f'{name}:{value:.3g}' for name, value in contributors)
...
def train_model(
    ...
    resume_model_class = None
    if resume_checkpoint_bundle is not None:
        resume_model_class = str(resume_checkpoint_bundle.get('model_class', '') or '')
        if 'input_dim' in resume_checkpoint_bundle:
            ...
            if first_weight is None:
                first_weight = resume_state.get('shared_trunk.0.weight')
    ...
    if resume_model_class == 'GlobalAdaptiveLakePINN':
    ...
    model = create_lake_model(
        ...
        hidden_dim=128,
        hidden_layers=8,
        adapter_dim=int((resume_checkpoint_bundle or {}).get('adapter_dim', 32) or 32),
        lake_attr_indices=(resume_checkpoint_bundle or {}).get('lake_attr_indices'),
    ).to(device)
    if hasattr(model, 'set_adaptation_mode'):
        model.set_adaptation_mode(adaptation_mode)
    ...
    weights = {
        ...
        'mid_deep_temporal_smoothness': float(mid_deep_temporal_smoothness_weight),
        'autumn_downward_cooling': float(autumn_downward_cooling_weight),
        'warm_deep_winter_weak_gradient': float(warm_deep_winter_weak_gradient_weight),
        'thermocline_shape': float(thermocline_shape_weight),
    ...
    if resume_checkpoint_bundle is not None:
        ...
        if optimizer_state is not None:
            try:
                ...
                print(
                    "Skipping optimizer state restore because the current "
                    f"adaptation mode changes trainable parameter groups: {adaptation_mode}"
                )
    ...
    history_forcing = {
        ...
        for name in (*HISTORY_FORCING_COLUMNS, *STATE_MEMORY_COLUMNS, *OPTIONAL_HYDRO_FEATURE_COLUMNS)
    ...
    time_step_seconds_np = (
        date_series.diff().dt.total_seconds().iloc[1:].to_numpy(dtype=np.float64, copy=False)
        if n_days > 1
        else np.empty((0,), dtype=np.float64)
    )
    time_step_seconds_np = np.where(
        np.isfinite(time_step_seconds_np) & (time_step_seconds_np > 0.0),
        time_step_seconds_np,
        SECONDS_PER_DAY,
    )
    ...
    window_losses = []
    loss_history_rows = []
    last_loss_contribution_row = None
    ...
    for epoch in range(epochs):
        ...
        if n_days > 1 and float(profile_grid_physics_weight) > 0.0:
            ...
            if (
                float(warm_deep_winter_weak_gradient_weight) > 0.0
                and is_geographic_warm_deep_lake(metadata, max_depth=max_depth)
            ):
                doy_for_grid = pd.to_datetime(df['Date']).dt.dayofyear.to_numpy(dtype=np.float32)
                winter_pair_candidates = np.flatnonzero(
                    (doy_for_grid[:-1] <= 75.0) | (doy_for_grid[:-1] >= 335.0)
                )
                if winter_pair_candidates.size > 0:
                    winter_pair_count = int(min(
                        winter_pair_candidates.size,
                        max(2, profile_grid_pair_count // 3),
                        profile_grid_pair_count,
                    ))
                    winter_index_pick = torch.randint(
                        low=0,
                        high=int(winter_pair_candidates.size),
                        size=(winter_pair_count,),
                        device=device,
                    )
                    winter_candidates_tensor = torch.as_tensor(
                        winter_pair_candidates,
                        dtype=torch.long,
                        device=device,
                    )
                    profile_grid_pick[:winter_pair_count] = winter_candidates_tensor[winter_index_pick]
        ...
        if n_days > 1:
            ...
            budget_relative_humidity_next = relative_humidity[budget_pick + 1]
            budget_surface_pressure = surface_pressure[budget_pick]
            budget_surface_pressure_next = surface_pressure[budget_pick + 1]
            budget_pick_np = budget_pick.detach().cpu().numpy()
            budget_doy_now_by_day = torch.tensor(
                pd.to_datetime(df['Date'].iloc[budget_pick_np]).dt.dayofyear.to_numpy(dtype=np.float32).reshape(-1, 1),
                dtype=torch.float32,
                device=device,
            )
            budget_doy_next_by_day = torch.tensor(
                pd.to_datetime(df['Date'].iloc[budget_pick_np + 1]).dt.dayofyear.to_numpy(dtype=np.float32).reshape(-1, 1),
                dtype=torch.float32,
                device=device,
            )
            ...
            budget_dz_line = torch.full_like(depth_line, budget_dz_value)
            if int(depth_line.shape[0]) > 1:
                budget_dz_line[0] = 0.5 * budget_dz_line[0]
                budget_dz_line[-1] = 0.5 * budget_dz_line[-1]
            budget_dz = budget_dz_line.repeat(budget_batch_size, 1)
            ...
            budget_relative_humidity_next = None
            budget_surface_pressure = None
            budget_surface_pressure_next = None
            budget_doy_now_by_day = None
            budget_doy_next_by_day = None
        ...
        batch = {
            ...
            'budget_relative_humidity_next': budget_relative_humidity_next,
            'budget_surface_pressure': budget_surface_pressure,
            'budget_surface_pressure_next': budget_surface_pressure_next,
            'budget_doy_now_by_day': budget_doy_now_by_day,
            'budget_doy_next_by_day': budget_doy_next_by_day,
        ...
        for name, values in history_forcing.items():
            ...
            if budget_surface_time is not None:
                ...
                # The heat-budget residual should measure physical heat-content
                # change over an interval, not jumps introduced by causal
                # observation/LST state-memory features.
                budget_next_values = values[budget_pick] if name in STATE_MEMORY_COLUMNS else values[budget_pick + 1]
                batch[f'budget_next_expanded_{name}'] = budget_next_values.repeat_interleave(depth_line.shape[0], dim=0)
            else:
        ...
        if budget_surface_time is not None:
            ...
            budget_history_kwargs_next = {
                name: (values[budget_pick] if name in STATE_MEMORY_COLUMNS else values[budget_pick + 1])
                for name, values in history_forcing.items()
            }
            ...
            budget_surface_temp_next = model_temperature(
                ...
                wind_speed=budget_wind_speed_next,
                shortwave=solar_flux[budget_pick + 1],
                lst_surface=budget_lst_surface_next,
                longwave=budget_longwave_next,
                latent_heat=budget_latent_heat_next,
                sensible_heat=budget_sensible_heat_next,
                secchi=budget_secchi_next,
                **budget_history_kwargs_next,
            )
            budget_flux_terms = compute_surface_flux_terms(
                budget_surface_temp,
                {
                    ...
                    'surface_longwave': budget_longwave,
                    'surface_latent_heat': budget_latent_heat,
                    'surface_sensible_heat': budget_sensible_heat,
                },
                shortwave_surface_fraction=shortwave_surface_fraction,
            )
            budget_flux_terms_next = compute_surface_flux_terms(
                budget_surface_temp_next,
                {
                    ...
                    'surface_relative_humidity': budget_relative_humidity_next,
                    'surface_pressure': budget_surface_pressure_next,
                    'surface_shortwave': solar_flux[budget_pick + 1],
                    'surface_longwave': budget_longwave_next,
                    'surface_latent_heat': budget_latent_heat_next,
                    'surface_sensible_heat': budget_sensible_heat_next,
                },
                shortwave_surface_fraction=shortwave_surface_fraction,
            )
            batch['budget_surface_flux'] = 0.5 * (
                budget_flux_terms['seb_flux'] + budget_flux_terms_next['seb_flux']
            )
            budget_surface_flux_wm2_now = (
                budget_flux_terms['net_radiation']
                - budget_flux_terms['sensible_heat']
                - budget_flux_terms['latent_heat']
            )
            budget_surface_flux_wm2_next = (
                budget_flux_terms_next['net_radiation']
                - budget_flux_terms_next['sensible_heat']
                - budget_flux_terms_next['latent_heat']
            )
            batch['budget_surface_flux_wm2_now'] = budget_surface_flux_wm2_now
            batch['budget_surface_flux_wm2_next'] = budget_surface_flux_wm2_next
            batch['budget_surface_flux_wm2'] = 0.5 * (
                budget_surface_flux_wm2_now + budget_surface_flux_wm2_next
            )
        else:
        ...
        loss_contribution_row = build_loss_contribution_row(epoch, losses, effective_weights)
        loss_history_rows.append(loss_contribution_row)
        last_loss_contribution_row = loss_contribution_row
            ...
            window_losses.append(
                {
                    ...
                    'loss_bc': losses['loss_bc'].item(),
                        ...
                        'profile_grid_mid_deep_jump_rms': losses['profile_grid_mid_deep_jump_rms'].item(),
                        'profile_grid_autumn_downward_cooling_rms': losses['profile_grid_autumn_downward_cooling_rms'].item(),
                        'profile_grid_warm_deep_winter_range_rms': losses['profile_grid_warm_deep_winter_range_rms'].item(),
                        'profile_grid_warm_deep_jan_deep_memory_rms': losses['profile_grid_warm_deep_jan_deep_memory_rms'].item(),
                        'profile_grid_thermocline_shape_rms': losses['profile_grid_thermocline_shape_rms'].item(),
        ...
        if epoch % 200 == 0 or epoch == epochs - 1:
            print(
                ...
                f"adv={losses['loss_vertical_exchange'].item():.4f} | conv={losses['loss_convective_mixing'].item():.4f} | mix={losses['loss_surface_mixed_layer_uniformity'].item():.4f} | drop={losses['loss_abrupt_surface_cooling'].item():.4f} | bottomslow={losses['loss_bottom_slow_change'].item():.4f} | overturn={losses['loss_autumn_overturn'].item():.4f} | heat={losses['loss_heat_budget'].item():.4f} | energy={losses['energy_budget_residual_rms_wm2'].item():.1f}Wm2 | grid={losses['loss_profile_grid_physics'].item():.4f} | "
                f"energy_parts_wm2=lhs:{losses['energy_budget_lhs_rms_wm2'].item():.1f}/rhs:{losses['energy_budget_rhs_rms_wm2'].item():.1f}/surf:{losses['energy_budget_surface_flux_rms_wm2'].item():.1f}/sw:{losses['energy_budget_internal_flux_rms_wm2'].item():.1f} | "
                f"grid_jump={losses['profile_grid_day_jump_rms'].item():.3f} | grid_surf={losses['profile_grid_surface_jump_rms'].item():.3f} | grid_col={losses['profile_grid_column_jump_rms'].item():.3f} | grid_middeep={losses['profile_grid_mid_deep_jump_rms'].item():.3f} | grid_downcool={losses['profile_grid_autumn_downward_cooling_rms'].item():.3f} | grid_winter_rng={losses['profile_grid_warm_deep_winter_range_rms'].item():.3f} | grid_janmem={losses['profile_grid_warm_deep_jan_deep_memory_rms'].item():.3f} | grid_thermo={losses['profile_grid_thermocline_shape_rms'].item():.3f} | grid_dens={losses['profile_grid_density_rms'].item():.3e} | "
                f"energy_season_wm2={losses['energy_budget_residual_rms_wm2_winter'].item():.1f}/{losses['energy_budget_residual_rms_wm2_spring'].item():.1f}/{losses['energy_budget_residual_rms_wm2_summer'].item():.1f}/{losses['energy_budget_residual_rms_wm2_autumn'].item():.1f} | "
                f"top_weighted={format_top_loss_contributors(loss_contribution_row)} | "
                ...
                f"adv_w={effective_weights['vertical_exchange']:.3f} | conv_w={effective_weights['convective_mixing']:.3f} | mix_w={effective_weights['surface_mixed_layer_uniformity']:.3f} | drop_w={effective_weights['abrupt_surface_cooling']:.3f} | bottomslow_w={effective_weights.get('bottom_slow_change', 0.0):.3f} | middeep_w={effective_weights.get('mid_deep_temporal_smoothness', 0.0):.3f} | downcool_w={effective_weights.get('autumn_downward_cooling', 0.0):.3f} | winterrng_w={effective_weights.get('warm_deep_winter_weak_gradient', 0.0):.3f} | thermo_w={effective_weights.get('thermocline_shape', 0.0):.3f} | overturn_w={effective_weights['autumn_overturn']:.3f} | heat_w={effective_weights['heat_budget']:.3f} | grid_w={effective_weights.get('profile_grid_physics', 0.0):.3f}"
            )
                ...
                ppo_history.append(
                    {
                        ...
                        'lambda_mid_deep_temporal_smoothness': effective_weights.get('mid_deep_temporal_smoothness', 0.0),
                        'lambda_autumn_downward_cooling': effective_weights.get('autumn_downward_cooling', 0.0),
                        'lambda_warm_deep_winter_weak_gradient': effective_weights.get('warm_deep_winter_weak_gradient', 0.0),
                        'lambda_thermocline_shape': effective_weights.get('thermocline_shape', 0.0),
    ...
    training_info = {
        ...
        'loss_history': pd.DataFrame(loss_history_rows),
        'final_loss_contributions': {} if last_loss_contribution_row is None else dict(last_loss_contribution_row),
        'loss_component_units': dict(LOSS_COMPONENT_UNITS),
        ...
        'model_architecture': str(getattr(model, 'model_class', model.__class__.__name__)),
        'adaptation_mode': str(getattr(model, 'adaptation_mode', adaptation_mode)),
    }
