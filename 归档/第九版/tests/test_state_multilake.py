...
from lake_pinn.state_multilake import (
    ...
    _select_support_dates_by_strategy,
    _select_segment_rollout_sequences,
    _sparse_observer_support_dates,
    _support_schedule_gap_summary,
    _transition_loss,
    ...
    evaluate_lake_pairs,
    evaluate_lake_fewshot_episodes,
...
def test_episodic_fewshot_sequences_use_only_prior_support_and_future_query_targets():
    dates = pd.date_range("2020-01-01", periods=80, freq="D")
    df = pd.DataFrame({"Date": dates})
    profile_dates = [dates[idx] for idx in [0, 10, 20, 40, 70]]
    profile_lookup = {
        pd.Timestamp(date).normalize(): np.array([5.0, 4.0], dtype=np.float32)
        for date in profile_dates
    }

    sequences = _build_episodic_fewshot_sequences(df, profile_lookup, max_query_days=45)

    assert sequences
    for query_start, query_start_idx, support_dates, targets in sequences:
        assert all(date < query_start for date in support_dates)
        assert all(target > query_start for target, _target_idx in targets)
        assert query_start not in support_dates
        assert all(1 <= int(target_idx - query_start_idx) <= 45 for _target, target_idx in targets)

    selected = _select_episodic_fewshot_sequences(
        sequences,
        active_max_days=30,
        samples_per_lake=2,
        epoch=0,
    ...
    assert selected
    for query_start, query_start_idx, support_dates, targets in selected:
        assert all(date < query_start for date in support_dates)
        assert all(1 <= int(target_idx - query_start_idx) <= 30 for _target, target_idx in targets)


def test_sparse_observer_support_schedule_p0_preserves_current_selection():
    base = pd.Timestamp("2020-01-01")
    support_offsets = [0, 10, 20, 50, 100, 200, 300]
    dates = pd.date_range(base, periods=365, freq="D")
    df = pd.DataFrame({"Date": dates})
    support_lookup = {
        base + pd.Timedelta(days=offset): np.array([5.0, 4.0], dtype=np.float32)
        for offset in support_offsets
    }

    selected = _sparse_observer_support_dates(
        df,
        support_lookup,
        rollout_start_idx=0,
        max_profile_count=3,
        min_gap_days=45,
        schedule_strategy="p0_current",
    )
    assert [int((date - base).days) for date in selected] == [0, 50, 100]

    dated_indices = [(base + pd.Timedelta(days=offset), offset) for offset in support_offsets]
    latest_selected = _select_support_dates_by_strategy(
        dated_indices,
        max_profile_count=3,
        min_gap_days=0,
        schedule_strategy="current",
        current_policy="latest",
    )
    assert [int((date - base).days) for date in latest_selected] == [100, 200, 300]


def test_schedule_aware_support_selection_moves_anchors_later_without_past_leakage():
    base = pd.Timestamp("2020-01-01")
    support_offsets = [0, 10, 20, 50, 100, 200, 300]
    dates = pd.date_range(base, periods=365, freq="D")
    df = pd.DataFrame({"Date": dates})
    ...
    date_to_index = {pd.Timestamp(date).normalize(): idx for idx, date in enumerate(dates)}

    even_selected = _sparse_observer_support_dates(
        df,
        support_lookup,
        rollout_start_idx=0,
        max_profile_count=3,
        min_gap_days=45,
        schedule_strategy="p1_even_calendar",
    )
    drift_selected = _sparse_observer_support_dates(
        df,
        support_lookup,
        rollout_start_idx=0,
        max_profile_count=3,
        min_gap_days=45,
        schedule_strategy="p3_drift_aware",
    )

    assert [int((date - base).days) for date in even_selected] == [50, 100, 200]
    assert [int((date - base).days) for date in drift_selected] == [100, 200, 300]
    assert _support_schedule_gap_summary(drift_selected, date_to_index) == {
        "max_gap_days": 100,
        "mean_gap_days": 100.0,
    }

    post_start_selected = _sparse_observer_support_dates(
        df,
        support_lookup,
        rollout_start_idx=60,
        max_profile_count=3,
        min_gap_days=45,
        schedule_strategy="p3_drift_aware",
    )
    assert [int((date - base).days) for date in post_start_selected] == [100, 200, 300]
    assert all(date_to_index[date] >= 60 for date in post_start_selected)

    with pytest.raises(ValueError, match="support schedule strategy"):
        _select_support_dates_by_strategy(
            [(base + pd.Timedelta(days=0), 0)],
            max_profile_count=1,
            min_gap_days=0,
...
def test_kd_saturation_penalty_only_counts_multiplier_above_threshold():
    diagnostics = {
        "nn_kd_multiplier": torch.tensor([1.40, 1.55, 1.80], dtype=torch.float32),
        "residual_abs_mean_c": torch.zeros(3, dtype=torch.float32),
    }

    loss = _kd_saturation_penalty_loss(diagnostics, threshold=1.55)
    per_sample = _kd_saturation_penalty_loss_per_sample(diagnostics, threshold=1.55)

    assert loss.item() == pytest.approx((0.25 ** 2) / 3.0)
    assert per_sample.tolist() == pytest.approx([0.0, 0.0, 0.25 ** 2])

    missing_kd = {"residual_abs_mean_c": torch.zeros(2, dtype=torch.float32)}
    assert _kd_saturation_penalty_loss(missing_kd, threshold=1.55).item() == pytest.approx(0.0)
    assert _kd_saturation_penalty_loss_per_sample(missing_kd, threshold=1.55).tolist() == pytest.approx([0.0, 0.0])


def test_sparse_observer_persistence_diagnostics_bins_depth_band_errors():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        ...
        for date in dates:
            for depth in depths:
                pred_rows.append({
                    ...
                    "Depth_m": depth,
                    "Temperature_C": 10.0 + depth * 0.01,
                })
        prediction_csv = root / "predictions.csv"
        pd.DataFrame(pred_rows).to_csv(prediction_csv, index=False)

        obs_rows = []
        for date in [pd.Timestamp("2020-01-04"), pd.Timestamp("2020-01-10"), pd.Timestamp("2020-01-20")]:
            for depth in depths:
                obs_rows.append({
                    "Date": date.date().isoformat(),
                    "Depth_m": depth,
                    "Temperature_C": 9.0 + depth * 0.01,
                })
        profile_csv = root / "profile.csv"
        pd.DataFrame(obs_rows).to_csv(profile_csv, index=False)

        diagnostics_df = pd.DataFrame({
            "Date": ["2020-01-04", "2020-01-10", "2020-01-20"],
            "sparse_observer_days_since_last_support": [3.0, 9.0, 19.0],
            "sparse_observer_days_until_next_support": [42.0, 36.0, 26.0],
            "sparse_observer_state_delta_mean_abs_c": [0.2, 0.3, 0.4],
            "sparse_observer_state_delta_heat_content_c": [0.1, 0.2, 0.3],
            "adaptive_kd_multiplier": [1.1, 1.2, 1.3],
            "kz_mean": [0.01, 0.02, 0.03],
            "residual_abs_mean_c": [0.4, 0.5, 0.6],
            "energy_residual_wm2": [5.0, 6.0, 7.0],
            "density_adjustment_max_delta_c": [0.0, 0.1, 0.2],
            "sparse_observer_support_schedule_strategy": ["p1_even_calendar"] * 3,
        })
        lake = {
            "lake_id": "lake_a",
            "profile_obs_path": str(profile_csv),
            "metadata": {"reservoir_indicator": 0.0},
        }

        result = _write_sparse_observer_persistence_diagnostics(
            lake,
            prediction_csv,
            diagnostics_df,
            root,
            {"file_tag": "lake_a"},
            "heldout_state_reconstruction_sparse_observer",
        )

        summary_path = result["support_persistence_summary_csv"]
        point_path = result["support_persistence_point_csv"]
        ...
        assert len(points) == 9
        assert set(points["lake_type"]) == {"natural"}
        assert points["abs_error_c"].mean() == pytest.approx(1.0)
        summary = pd.read_csv(summary_path)
        assert {7, 14, 30}.issubset(set(summary["bin_end_days"]))
        whole_7 = summary[(summary["bin_end_days"] == 7) & (summary["depth_band"] == "whole")].iloc[0]
        le25_7 = summary[(summary["bin_end_days"] == 7) & (summary["depth_band"] == "le25m")].iloc[0]
        surface_7 = summary[(summary["bin_end_days"] == 7) & (summary["depth_band"] == "surface_le1m")].iloc[0]
        gt25_7 = summary[(summary["bin_end_days"] == 7) & (summary["depth_band"] == "gt25m")].iloc[0]
        assert whole_7["rmse_c"] == pytest.approx(1.0)
        assert whole_7["point_count"] == 3
        assert le25_7["point_count"] == 2
        assert surface_7["point_count"] == 1
        assert gt25_7["point_count"] == 1
        assert whole_7["kd_multiplier_mean"] == pytest.approx(1.1)


def test_fewshot_encoder_removed_from_zero_profile_mainline():
    with tempfile.TemporaryDirectory() as tmp:
        lake, model = _single_lake_and_model(Path(tmp))
        support_dates = sorted(lake["lookups"]["train"])[:3]
        support_profiles = []
        support_masks = []
        support_ages = []
        query_idx = lake["date_to_index"][support_dates[-1]] + 1
        for date in support_dates:
            profile = lake["lookup_tensors"]["train"][date].reshape(-1)
            mask = lake["lookup_mask_tensors"]["train"][date].reshape(-1).float()
            support_profiles.append(profile)
            support_masks.append(mask)
            support_ages.append(float(query_idx - lake["date_to_index"][date]))

        assert model.fewshot_encoder is None
        assert model.fewshot_adapter_params == "off"
        assert model.fewshot_adapter_scale == 0.0
        with pytest.raises(RuntimeError, match="few-shot support encoding has been removed"):
            model.encode_fewshot_support(
                torch.stack(support_profiles).unsqueeze(0),
                torch.stack(support_masks).unsqueeze(0),
                torch.tensor(support_ages, dtype=torch.float32).unsqueeze(0),
                lake["static_features"],
                lake["forcing_rows"][query_idx],
            )
        assert not any("lake_id" in name.lower() for name, _param in model.named_parameters())


def test_removed_fewshot_encoder_rejects_lake_specific_support_depth_count():
    with tempfile.TemporaryDirectory() as tmp:
        lake, model = _single_lake_and_model(Path(tmp))
        support_dates = sorted(lake["lookups"]["train"])[:3]
        source_depth_count = int(lake["depths"].numel())
        support_depth_count = source_depth_count + 3
        support_profiles = []
        support_masks = []
        support_ages = []
        query_idx = lake["date_to_index"][support_dates[-1]] + 1
        for date in support_dates:
            profile = lake["lookup_tensors"]["train"][date].reshape(1, 1, -1)
            mask = lake["lookup_mask_tensors"]["train"][date].reshape(1, 1, -1).float()
            support_profiles.append(
                torch.nn.functional.interpolate(
                    profile,
                    size=support_depth_count,
                    mode="linear",
                    align_corners=True,
                ).reshape(-1)
            )
...
def test_evaluate_lake_fewshot_episodes_scores_support_conditioned_queries():
    with tempfile.TemporaryDirectory() as tmp:
        lake, model = _single_lake_and_model(Path(tmp))

        metrics = evaluate_lake_fewshot_episodes(
            model,
            lake,
            horizons=(5,),
        ...
        assert "rmse_5d" in metrics
        assert metrics["count_5d"] == 0
        assert np.isnan(metrics["rmse_5d"])
...
def test_mainline_lswt_observer_modes_are_pruned():
    assert MAINLINE_LSWT_OBSERVER_MODES == {
        "off",
        "conservative_surface",
        "conservative_mld_shallow",
    }
    assert normalize_mainline_lswt_observer_mode("off") == "off"
    assert (
        normalize_mainline_lswt_observer_mode("r20_conservative_surface")
        == "conservative_surface"
    )
    assert (
        normalize_mainline_lswt_observer_mode("conservative_mld")
        == "conservative_mld_shallow"
    )
    ...
    for removed_mode in [
        ...
        with pytest.raises(ValueError, match="removed from the mainline"):
            normalize_mainline_lswt_observer_mode(removed_mode)

    with pytest.raises(ValueError, match="rejected/legacy diagnostic"):
        normalize_mainline_lswt_observer_mode("mld_heat_content")


def test_mainline_zero_profile_initializer_modes_are_pruned():
    assert MAINLINE_ZERO_PROFILE_INITIALIZER_MODES == {
        "low_dof",
        "lswt_climatology_low_dof",
        "eof_pca_low_dof",
    }
    assert normalize_mainline_zero_profile_initializer_mode("lowdof") == "low_dof"
    assert (
        normalize_mainline_zero_profile_initializer_mode("lswt_climatology")
        == "lswt_climatology_low_dof"
    )
    assert (
        normalize_mainline_zero_profile_initializer_mode("thermal_state_low_dof")
        == "eof_pca_low_dof"
    )
    with pytest.raises(ValueError, match="removed from the mainline"):
        normalize_mainline_zero_profile_initializer_mode("legacy_prior")


def test_low_rank_lswt_observer_uses_only_raw_open_water_and_caps_deep_update():
    profile = torch.tensor([[10.0, 9.5, 8.0, 7.0]], dtype=torch.float32)
    depths = torch.tensor([0.0, 2.0, 10.0, 30.0], dtype=torch.float32)
    area = torch.ones_like(depths)
    filled_row = {
        "lswt_open_water": torch.tensor([20.0]),
        "lst_quality": torch.tensor([1.0]),
        "lst_observed_flag": torch.tensor([0.0]),
        "lst_is_filled": torch.tensor([1.0]),
        "ice_mask": torch.tensor([0.0]),
        "ice_fraction": torch.tensor([0.0]),
    }

    skipped, skipped_detail = apply_lswt_observer_update(
        profile,
        filled_row,
        depths,
        mode="low_rank",
        strength=0.5,
        max_increment_c=1.0,
        heat_content_limit_c=0.2,
        area_profile=area,
    )

    assert torch.allclose(skipped, profile)
    assert float(skipped_detail["lswt_observer_applied_count"]) == 0.0
    assert float(skipped_detail["lswt_observer_filled_lst_used_count"]) == 0.0

    observed_row = dict(filled_row)
    observed_row["lst_observed_flag"] = torch.tensor([1.0])
    observed_row["lst_is_filled"] = torch.tensor([0.0])
    updated, detail = apply_lswt_observer_update(
        ...
        max_increment_c=1.0,
        heat_content_limit_c=0.2,
        low_rank_deep_update_fraction=0.10,
        area_profile=area,
    )
    delta = updated - profile

    assert float(detail["lswt_observer_applied_count"]) == pytest.approx(1.0)
    assert float(detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert delta[0, 0] > 0.0
    assert abs(float(delta[0, -1])) < abs(float(delta[0, 0]))
    assert abs(float(detail["lswt_observer_heat_content_delta_c"])) <= 0.2001


def test_conservative_surface_lswt_observer_stays_shallow_and_scales_reservoir():
    profile = torch.tensor([[10.0, 9.7, 8.8, 7.0, 6.5]], dtype=torch.float32)
    depths = torch.tensor([0.0, 1.0, 3.0, 8.0, 30.0], dtype=torch.float32)
    area = torch.ones_like(depths)
    filled_row = {
        "lswt_open_water": torch.tensor([14.0]),
        "lst_quality": torch.tensor([0.95]),
        "lst_observed_flag": torch.tensor([0.0]),
        "lst_is_filled": torch.tensor([1.0]),
        "ice_mask": torch.tensor([0.0]),
        "ice_fraction": torch.tensor([0.0]),
    ...
    observed_row["lst_observed_flag"] = torch.tensor([1.0])
    observed_row["lst_is_filled"] = torch.tensor([0.0])
    natural, natural_detail = apply_lswt_observer_update(
        profile,
        observed_row,
        depths,
        mode="conservative_surface",
    ...
    reservoir, reservoir_detail = apply_lswt_observer_update(
        profile,
        observed_row,
        depths,
        mode="conservative_surface",
        ...
        heat_content_limit_c=0.10,
        area_profile=area,
        metadata={"reservoir_indicator": 1.0},
    )
    natural_delta = natural - profile
    reservoir_delta = reservoir - profile

    assert float(natural_detail["lswt_observer_applied_count"]) == pytest.approx(1.0)
    assert float(natural_detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert float(natural_detail["lswt_observer_localization_depth_m"]) <= 3.0
    assert float(natural_detail["lswt_observer_kalman_gain_surface"]) > 0.0
    assert natural_delta[0, 0] > 0.0
    assert abs(float(natural_delta[0, 2])) < abs(float(natural_delta[0, 0]))
    assert abs(float(natural_delta[0, -1])) == pytest.approx(0.0)
    assert abs(float(natural_detail["lswt_observer_heat_content_delta_c"])) <= 0.1001
    assert float(reservoir_detail["lswt_observer_reservoir_conservative_scale"]) == pytest.approx(0.35)
    assert 0.0 < float(reservoir_delta[0, 0]) < float(natural_delta[0, 0])


def test_conservative_mld_shallow_observer_uses_raw_lswt_and_blocks_deep_update():
    profile = torch.tensor([[10.0, 9.9, 9.6, 6.5, 5.0]], dtype=torch.float32)
    depths = torch.tensor([0.0, 2.0, 6.0, 14.0, 30.0], dtype=torch.float32)
    area = torch.ones_like(depths)
    filled_row = {
        "lswt_open_water": torch.tensor([14.0]),
        "lst_quality": torch.tensor([0.95]),
    ...
    skipped, skipped_detail = apply_lswt_observer_update(
        ...
        filled_row,
        depths,
        mode="conservative_mld_shallow",
        strength=1.0,
        max_increment_c=2.0,
        heat_content_limit_c=0.50,
        low_rank_deep_update_fraction=0.20,
        area_profile=area,
    )
    ...
    assert float(natural_detail["lswt_observer_applied_count"]) == pytest.approx(1.0)
    assert float(natural_detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert float(natural_detail["lswt_observer_mld_depth_m"]) > 1.0
    assert float(natural_detail["lswt_observer_mld_weight_mean"]) > 0.0
    assert float(natural_detail["lswt_observer_mld_surface_to_heat_gain"]) > 0.0
    assert natural_delta[0, 0] > 0.0
    assert natural_delta[0, 1] > 0.0
    assert abs(float(natural_delta[0, -1])) == pytest.approx(0.0)
    assert abs(float(natural_detail["lswt_observer_deep_abs_delta_c"])) == pytest.approx(0.0)
    assert abs(float(natural_detail["lswt_observer_heat_content_delta_c"])) <= 0.0801
    assert float(reservoir_detail["lswt_observer_reservoir_conservative_scale"]) == pytest.approx(0.25)
    assert 0.0 < float(reservoir_delta[0, 0]) < float(natural_delta[0, 0])


def test_enkf_low_rank_lswt_observer_uses_kalman_gain_and_respects_gates():
    profile = torch.tensor([[10.0, 9.5, 8.0, 7.0]], dtype=torch.float32)
    depths = torch.tensor([0.0, 2.0, 10.0, 30.0], dtype=torch.float32)
    area = torch.ones_like(depths)
    filled_row = {
        "lswt_open_water": torch.tensor([18.0]),
        "lst_quality": torch.tensor([0.9]),
        "lst_observed_flag": torch.tensor([0.0]),
        "lst_is_filled": torch.tensor([1.0]),
        "ice_mask": torch.tensor([0.0]),
        "ice_fraction": torch.tensor([0.0]),
    ...
    assert float(skipped_detail["lswt_observer_applied_count"]) == 0.0
    assert float(skipped_detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert float(skipped_detail["lswt_observer_kalman_gain_surface"]) == 0.0
    ...
    assert float(detail["lswt_observer_applied_count"]) == pytest.approx(1.0)
    assert float(detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert float(detail["lswt_observer_kalman_gain_surface"]) > 0.0
    assert float(detail["lswt_observer_kalman_gain_mean"]) > 0.0
    assert float(detail["lswt_observer_observation_error_c"]) > 0.0
    assert float(detail["lswt_observer_state_variance_surface"]) > 0.0
    assert float(detail["lswt_observer_localization_depth_m"]) > 0.0
    assert float(detail["lswt_observer_reservoir_conservative_scale"]) == pytest.approx(0.65)
    assert delta[0, 0] > 0.0
    assert abs(float(delta[0, -1])) < abs(float(delta[0, 0]))
    assert abs(float(detail["lswt_observer_heat_content_delta_c"])) <= 0.1501
    assert 0.0 < float(detail["lswt_observer_heat_content_bound_scale"]) <= 1.0


def test_mld_heat_content_lswt_observer_routes_innovation_into_mixed_layer():
    profile = torch.tensor([[10.0, 9.8, 9.6, 5.0]], dtype=torch.float32)
    depths = torch.tensor([0.0, 2.0, 5.0, 20.0], dtype=torch.float32)
    area = torch.ones_like(depths)
    filled_row = {
        "lswt_open_water": torch.tensor([12.0]),
        "lst_quality": torch.tensor([0.9]),
        "lst_observed_flag": torch.tensor([0.0]),
        "lst_is_filled": torch.tensor([1.0]),
        "ice_mask": torch.tensor([0.0]),
    ...
    assert float(skipped_detail["lswt_observer_applied_count"]) == 0.0
    assert float(skipped_detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert float(skipped_detail["lswt_observer_mld_depth_m"]) == 0.0
    ...
    updated, detail = apply_lswt_observer_update(
        ...
        max_increment_c=1.0,
        heat_content_limit_c=0.2,
        low_rank_deep_update_fraction=0.05,
        area_profile=area,
    )
    delta = updated - profile
    ...
    assert float(detail["lswt_observer_applied_count"]) == pytest.approx(1.0)
    assert float(detail["lswt_observer_filled_lst_used_count"]) == 0.0
    assert float(detail["lswt_observer_mld_depth_m"]) >= 2.0
    assert float(detail["lswt_observer_mld_weight_mean"]) > 0.0
    assert float(detail["lswt_observer_mld_volume_fraction"]) > 0.0
    assert float(detail["lswt_observer_mld_surface_to_heat_gain"]) > 0.0
    assert float(detail["lswt_observer_mld_heat_content_delta_c"]) > 0.0
    assert delta[0, 0] > 0.0
    assert abs(float(delta[0, 1])) > 0.5 * abs(float(delta[0, 0]))
    assert abs(float(delta[0, -1])) < abs(float(delta[0, 0]))
    assert abs(float(detail["lswt_observer_heat_content_delta_c"])) <= 0.2001


def test_zero_profile_low_dof_prior_uses_no_profile_and_records_metadata():
    dates = pd.date_range("2020-01-01", periods=12, freq="D")
    df = pd.DataFrame({
        "Date": dates,
        "T_air_C": np.linspace(2.0, 8.0, len(dates)),
        "Solar_W_m2": np.linspace(80.0, 160.0, len(dates)),
        "LSWT_open_water_C": np.linspace(5.0, 7.0, len(dates)),
        "LST_surface_C": np.linspace(5.2, 7.2, len(dates)),
        "full_doy": [date.dayofyear for date in dates],
        "ice_mask": [0.0] * len(dates),
    })
    profile, info = build_zero_profile_low_dof_prior(
        df,
        np.array([0.0, 2.0, 6.0, 12.0], dtype=np.float32),
        {"max_depth_m": 12.0, "mean_depth_m": 5.0, "latitude": 45.0, "secchi_m": 2.0},
        start_idx=10,
    )

    assert profile.shape == (4,)
    assert np.all(np.isfinite(profile))
    assert info["zero_profile_initializer"] == "low_dof"
    assert np.isfinite(info["zero_profile_recent_lswt_30d_c"])


def test_lswt_climatology_low_dof_prior_ignores_filled_lst_as_strong_anchor():
    dates = pd.date_range("2020-07-01", periods=20, freq="D")
    df = pd.DataFrame({
        "Date": dates,
        "T_air_C": np.linspace(6.0, 8.0, len(dates)),
        "Solar_W_m2": np.linspace(120.0, 180.0, len(dates)),
        "wind_speed_m_per_s": [3.5] * len(dates),
        "LSWT_open_water_C": [np.nan] * len(dates),
        "LST_surface_C": [28.0] * len(dates),
        "LST_is_filled": [1.0] * len(dates),
        "LST_observed_flag": [0.0] * len(dates),
        "LST_qc_good_fraction": [1.0] * len(dates),
        "full_doy": [date.dayofyear for date in dates],
        "ice_mask": [0.0] * len(dates),
        "ice_fraction": [0.0] * len(dates),
    })
    profile_no_raw, info_no_raw = build_zero_profile_lswt_climatology_low_dof_prior(
        df,
        np.array([0.0, 2.0, 8.0, 20.0], dtype=np.float32),
        {"max_depth_m": 20.0, "mean_depth_m": 9.0, "latitude": 45.0, "secchi_m": 2.0},
        start_idx=19,
    )

    assert info_no_raw["zero_profile_initializer"] == "lswt_climatology_low_dof"
    assert info_no_raw["zero_profile_raw_lswt_count"] == 0
    assert info_no_raw["zero_profile_filled_lst_strong_target_used"] is False
    assert profile_no_raw[0] < 16.0

    observed = df.copy()
    observed.loc[12:, "LSWT_open_water_C"] = np.linspace(9.0, 10.5, len(observed.loc[12:]))
    observed.loc[12:, "LST_surface_C"] = observed.loc[12:, "LSWT_open_water_C"]
    observed.loc[12:, "LST_is_filled"] = 0.0
    observed.loc[12:, "LST_observed_flag"] = 1.0
    profile_raw, info_raw = build_zero_profile_lswt_climatology_low_dof_prior(
        observed,
        np.array([0.0, 2.0, 8.0, 20.0], dtype=np.float32),
        {"max_depth_m": 20.0, "mean_depth_m": 9.0, "latitude": 45.0, "secchi_m": 2.0},
        start_idx=19,
    ...
    assert info_raw["zero_profile_raw_lswt_count"] > 0
    assert np.isfinite(info_raw["zero_profile_raw_lswt_anchor_c"])
    assert profile_raw[0] > profile_no_raw[0]
    assert profile_raw[0] < 16.0


def test_eof_pca_low_dof_prior_uses_train_basis_without_profile_input():
    depths = np.array([0.0, 2.0, 8.0, 20.0], dtype=np.float32)
    train_lookup = {
        pd.Timestamp("2020-07-01"): np.array([18.0, 17.5, 10.0, 6.0], dtype=np.float32),
        pd.Timestamp("2020-08-01"): np.array([21.0, 20.0, 12.0, 6.5], dtype=np.float32),
        pd.Timestamp("2020-09-01"): np.array([15.0, 14.5, 9.0, 6.0], dtype=np.float32),
    }
    basis = fit_zero_profile_eof_pca_basis(
        [{
            "lake_id": "train_lake",
            "depths": depths,
            "lookup": train_lookup,
            "masks": {date: np.ones_like(depths, dtype=bool) for date in train_lookup},
        }],
        n_components=2,
        grid_points=8,
    )
    assert basis is not None
    assert basis["profile_count"] == 3
    assert basis["source_lake_count"] == 1

    dates = pd.date_range("2021-07-01", periods=20, freq="D")
    df = pd.DataFrame({
        "Date": dates,
        "T_air_C": np.linspace(14.0, 18.0, len(dates)),
        "Solar_W_m2": np.linspace(160.0, 220.0, len(dates)),
        "wind_speed_m_per_s": [2.8] * len(dates),
        "LSWT_open_water_C": np.linspace(16.0, 18.0, len(dates)),
        "LST_surface_C": np.linspace(16.0, 18.0, len(dates)),
        "LST_is_filled": [0.0] * len(dates),
        "LST_observed_flag": [1.0] * len(dates),
        "LST_qc_good_fraction": [1.0] * len(dates),
        "full_doy": [date.dayofyear for date in dates],
        "ice_mask": [0.0] * len(dates),
        "ice_fraction": [0.0] * len(dates),
    })
    profile, info = build_zero_profile_eof_pca_low_dof_prior(
        df,
        depths,
        {"max_depth_m": 20.0, "mean_depth_m": 9.0, "latitude": 45.0, "secchi_m": 2.0},
        start_idx=19,
    ...
    assert profile.shape == depths.shape
    assert np.all(np.isfinite(profile))
    assert info["zero_profile_initializer"] == "eof_pca_low_dof"
    assert info["zero_profile_base_initializer"] == "lswt_climatology_low_dof"
    assert info["zero_profile_thermal_basis_applied"] is True
    assert info["zero_profile_profile_derived_input_used"] is False
    assert info["zero_profile_filled_lst_strong_target_used"] is False
    assert info["thermal_basis_component_count"] == 2
...
def test_multilake_state_training_excludes_heldout_lake_from_checkpoint():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "fewshot_mainline_disabled" not in bundle
        assert not any(key.startswith("episodic_fewshot") for key in bundle)
        assert not any(key.startswith("fewshot_") for key in bundle)
        assert not any(key.startswith("support_persistence") for key in bundle)
        assert not any(key.startswith("sparse_observer") for key in bundle)
        ...
        assert "train_supervision_episodic_fewshot_sequence_count" not in history.columns
        ...
        assert not any(column.startswith("episodic_fewshot") for column in history.columns)
        assert not any(column.startswith("fewshot_") for column in history.columns)
        assert not any(column.startswith("support_persistence") for column in history.columns)
        assert not any(column.startswith("sparse_observer") for column in history.columns)
        ...
        assert "episodic_seconds" not in history.columns
        ...
        assert "val_fewshot_rmse_30d" not in history.columns
        assert "val_fewshot_count_30d" not in history.columns
        assert "heldout_fewshot_rmse_30d" not in history.columns
        assert "heldout_fewshot_count_30d" not in history.columns
        ...
        assert "fewshot_mainline_disabled" not in history.columns
        assert "episodic_fewshot_mode" not in history.columns
        assert "episodic_fewshot_weight_eff" not in history.columns
        ...
        assert "train_episodic_fewshot_sequences" not in split_summary["lake_a"]
        assert "val_episodic_fewshot_sequences" not in split_summary["lake_a"]
        assert "all_episodic_fewshot_sequences" not in split_summary["lake_a"]
        ...
        assert "supervision_episodic_fewshot_sequences" not in split_summary["lake_a"]
        assert split_summary["_config"]["test_lake_ids"] == ["lake_b"]
        assert split_summary["_config"]["seed"] == 123
        ...
        assert "fewshot_mainline_disabled" not in split_summary["_config"]
        assert not any(key.startswith("episodic_fewshot") for key in split_summary["_config"])
        assert not any(key.startswith("fewshot_") for key in split_summary["_config"])
        assert not any(key.startswith("support_persistence") for key in split_summary["_config"])
        assert not any(key.startswith("sparse_observer") for key in split_summary["_config"])
...
def test_legacy_episodic_fewshot_manifest_fields_are_rejected():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        manifest = {
            "split_mode": "time_blocked",
            "episodic_fewshot_mode": "on",
            "episodic_fewshot_loss_weight": 0.5,
            "episodic_fewshot_samples_per_lake": 4,
            "support_persistence_loss_weight": 0.2,
            "fewshot_adapter_scale": 0.25,
            "fewshot_adapter_params": "kd,kz",
            "lakes": [_write_lake_inputs(root, "lake_a", 0.0)],
        }
        manifest_path = root / "manifest.json"
        manifest_path.write_text(json.dumps(manifest), encoding="utf-8")

        with pytest.raises(ValueError, match="Few-shot/support-profile mainline fields were removed"):
            train_multilake_state_forecaster(
                manifest_path,
                root / "out",
                epochs=1,
...
def test_legacy_lswt_observer_manifest_modes_are_rejected_from_mainline():
    for mode in ["surface", "low_rank", "enkf_low_rank", "mld_heat_content"]:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            manifest = {
                "split_mode": "time_blocked",
                "zero_profile_lswt_observer_mode": mode,
                "lakes": [_write_lake_inputs(root, "lake_a", 0.0)],
            }
            manifest_path = root / "manifest.json"
            manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
...
def test_legacy_zero_profile_initializer_manifest_mode_is_rejected_from_mainline():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        manifest = {
            "split_mode": "time_blocked",
        ...
        with pytest.raises(ValueError, match="legacy_prior is retained only"):
            train_multilake_state_forecaster(
                manifest_path,
                root / "out",
                epochs=1,
...
def test_removed_support_profile_runtime_switches_are_rejected():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        manifest = {
            "split_mode": "time_blocked",
        ...
        with pytest.raises(ValueError, match="runtime switches were removed"):
            train_multilake_state_forecaster(
                manifest_path,
                root / "out",
                epochs=1,
...
def test_legacy_fewshot_checkpoint_tensors_are_filtered_from_zero_profile_model():
    with tempfile.TemporaryDirectory() as tmp:
        _lake, model = _single_lake_and_model(Path(tmp))
        state_dict = dict(model.state_dict())
        state_dict["fewshot_encoder.net.0.weight"] = torch.ones(4, 4)
        state_dict["fewshot_encoder.net.0.bias"] = torch.ones(4)

        filtered = _filter_state_forecaster_state_dict_for_load(model, state_dict)

        assert not any(key.startswith("fewshot_encoder.") for key in filtered)
        incompatible = model.load_state_dict(filtered, strict=False)
        assert not any(key.startswith("fewshot_encoder.") for key in incompatible.unexpected_keys)
...
def test_full_eval_point_diagnostics_write_free_roll_residual_csvs():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        manifest = {
            "split_mode": "time_blocked",
        ...
        result = train_multilake_state_forecaster(
            ...
            rolling_horizon_eval_max_starts=2,
            export_style_validation="on",
            export_style_validation_max_lakes=1,
            full_eval_point_diagnostics="on",
            zero_profile_export_validation="on",
            zero_profile_export_validation_max_lakes=1,
            zero_profile_initializer="low_dof",
            zero_profile_lswt_observer_mode="conservative_mld_shallow",
            zero_profile_spinup_days_matrix="0,1",
            zero_profile_lswt_observer_max_increment_c=0.4,
            zero_profile_lswt_observer_heat_content_limit_c=0.2,
            spinup_days=1,
            device="cpu",
        )
        ...
        assert final_row["eval_mode"] == "full"
        assert final_row["full_eval_point_diagnostics"] == "on"
        assert final_row["val_export_style_free_roll_point_diagnostics_count"] > 0
        assert final_row["heldout_free_roll_point_diagnostics_count"] > 0
        assert final_row["zero_profile_export_validation"] == "on"
        assert final_row["zero_profile_initializer"] == "low_dof"
        assert final_row["zero_profile_lswt_observer_mode"] == "conservative_mld_shallow"
        assert str(final_row["zero_profile_spinup_days_matrix"]) == "0,1"
        assert final_row["val_zero_profile_export_init_mode"] == "prior_spinup"
        assert final_row["val_zero_profile_export_rollout_mode"] == "free"
        assert final_row["val_zero_profile_export_initializer"] == "low_dof"
        assert (
            final_row["val_zero_profile_export_rollout_lswt_observer_mode"]
            == "conservative_mld_shallow"
        )
        assert final_row["val_zero_profile_export_lswt_observer_filled_lst_used_count"] == 0.0
        assert final_row["val_zero_profile_export_profile_input_at_inference"] in {False, "False"}
        assert final_row["val_zero_profile_export_support_input_at_inference"] in {False, "False"}
        assert final_row["val_zero_profile_export_mean_rmse"] > 0
        assert final_row["val_zero_profile_export_surface_count"] > 0
        assert final_row["heldout_zero_profile_export_mean_rmse"] > 0
        assert final_row["zero_profile_export_heldout_diagnostic_only"] in {True, "True"}
        assert final_row["val_zp_spinup0d_zero_profile_export_lake_count"] > 0

        val_point_path = Path(
            str(final_row["val_export_style_free_roll_point_diagnostics_csvs"]).split(";")[0]
        )
        heldout_summary_path = Path(
            str(final_row["heldout_free_roll_age_summary_csvs"]).split(";")[0]
        )
        zero_profile_point_path = Path(
            str(final_row["val_zero_profile_export_point_diagnostics_csvs"]).split(";")[0]
        )
        assert val_point_path.exists()
        assert heldout_summary_path.exists()
        assert zero_profile_point_path.exists()

        points = pd.read_csv(val_point_path)
        assert {
            "lake_id",
            "lake_type",
            "eval_split",
            ...
            "error_c",
            "abs_error_c",
            "days_since_rollout_start",
        }.issubset(points.columns)
        assert set(points["eval_split"]) == {"val_export_style"}
        zero_profile_points = pd.read_csv(zero_profile_point_path)
        assert set(zero_profile_points["eval_split"]) == {"val_zero_profile_export"}
        assert set(zero_profile_points["rollout_mode"]) == {"zero_profile_free"}
        if "scheduled_sparse_observer_support_schedule_strategy" in zero_profile_points.columns:
            assert (
                zero_profile_points["scheduled_sparse_observer_support_schedule_strategy"]
                .fillna("")
                .eq("")
                .all()
            )
        summary = pd.read_csv(heldout_summary_path)
        assert {"age_axis", "depth_band", "rmse_c", "point_count"}.issubset(summary.columns)
        assert "days_since_rollout_start" in set(summary["age_axis"])

        bundle = torch.load(result["checkpoint_path"], map_location="cpu")
        split_summary = json.loads(result["split_summary"].read_text(encoding="utf-8"))
        assert bundle["full_eval_point_diagnostics"] == "on"
        assert bundle["zero_profile_export_validation"] == "on"
        assert bundle["zero_profile_initializer"] == "low_dof"
        assert bundle["zero_profile_lswt_observer_mode"] == "conservative_mld_shallow"
        assert split_summary["_config"]["full_eval_point_diagnostics"] == "on"
        assert split_summary["_config"]["zero_profile_export_validation"] == "on"
        assert split_summary["_config"]["zero_profile_initializer"] == "low_dof"
        assert (
            split_summary["_config"]["zero_profile_lswt_observer_mode"]
            == "conservative_mld_shallow"
        )


def test_eof_pca_zero_profile_initializer_records_train_only_basis():
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        manifest = {
            "split_mode": "time_blocked",
        ...
        result = train_multilake_state_forecaster(
            ...
            zero_profile_thermal_basis_grid_points=8,
            device="cpu",
        )

        history = pd.read_csv(result["history_csv"])
        final_row = history.iloc[-1]
        assert final_row["zero_profile_initializer"] == "eof_pca_low_dof"
        assert int(final_row["zero_profile_thermal_basis_components"]) == 2
        assert int(final_row["zero_profile_thermal_basis_grid_points"]) == 8
        assert int(final_row["zero_profile_thermal_basis_profile_count"]) > 0
        bundle = torch.load(result["checkpoint_path"], map_location="cpu")
        assert bundle["zero_profile_initializer"] == "eof_pca_low_dof"
        assert bundle["zero_profile_thermal_basis"]["source_split"] == "train"
        assert bundle["zero_profile_thermal_basis"]["source_lake_count"] == 1
        split_summary = json.loads(result["split_summary"].read_text(encoding="utf-8"))
        assert split_summary["_config"]["zero_profile_initializer"] == "eof_pca_low_dof"
        assert int(split_summary["_config"]["zero_profile_thermal_basis_profile_count"]) > 0
...
def test_depth_stratified_rmse_splits_shallow_and_deep_bands():
    errors = {
        "le25m": [1.0, 4.0],
        "gt25m": [9.0, 16.0],
    }
    summary = _depth_rmse_record_from_squared_errors(errors)

    assert summary["rmse_le25m"] == pytest.approx(np.sqrt(2.5))
    assert summary["count_le25m"] == 2
    assert summary["rmse_gt25m"] == pytest.approx(np.sqrt(12.5))
    assert summary["count_gt25m"] == 2


def test_horizon_metric_record_includes_25m_depth_bands():
    depth_errors = _empty_depth_horizon_errors((30, 60))
    _extend_depth_horizon_errors(
        depth_errors,
        30,
        squared_errors=np.asarray([1.0, 4.0, 9.0, 16.0]),
        depths=np.asarray([0.0, 25.0, 26.0, 60.0]),
    )
    record = _horizon_metric_record(
        {30: [1.0, 4.0, 9.0, 16.0], 60: []},
        depth_errors_by_horizon=depth_errors,
    )

    assert record["rmse_30d"] == pytest.approx(np.sqrt(7.5))
    assert record["rmse_le25m_30d"] == pytest.approx(np.sqrt(2.5))
    assert record["count_le25m_30d"] == 2
    assert record["rmse_gt25m_30d"] == pytest.approx(np.sqrt(12.5))
    assert record["count_gt25m_30d"] == 2
    assert np.isnan(record["rmse_gt25m_60d"])
    assert record["count_gt25m_60d"] == 0
