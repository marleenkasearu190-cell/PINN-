...
def plot_year_heatmap(df, temp_grid, depths, output_path, metadata):
    ...
    day_min = float(np.nanmin(day_axis))
    day_max = float(np.nanmax(day_axis))
    ...
    if day_span >= 300.0:
        ax.text(25, max_depth * 0.10, 'Winter\nInverse\nStratification', color='blue', fontsize=22, fontweight='bold', ha='center', clip_on=True)
        ax.text(120, max_depth * 0.88, 'Spring\nWarming', color='green', fontsize=22, fontweight='bold', ha='center', clip_on=True)
        ax.text(210, max_depth * 0.78, 'Summer\nStratification\n(Thermocline)', color='red', fontsize=24, fontweight='bold', ha='center', clip_on=True)
        ax.text(305, max_depth * 0.52, 'Autumn\nOverturn\n(Homothermal)', color='black', fontsize=22, fontweight='bold', ha='center', clip_on=True)
