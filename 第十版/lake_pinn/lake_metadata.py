...
def metadata_static_features(metadata: dict | None, max_depth: float) -> dict:
    metadata = metadata or {}
    area_km2 = _safe_float(metadata.get('area_km2'), default=1.0)
    latitude = _safe_float(metadata.get('latitude'), default=45.0)
    longitude = _safe_float(metadata.get('longitude'), default=0.0)
    mean_depth_m = _safe_float(metadata.get('mean_depth_m'), default=0.0)
    if not np.isfinite(mean_depth_m) or mean_depth_m <= 0.0:
        mean_depth_m = 0.0
    volume_km3 = _safe_float(metadata.get('volume_km3'), default=0.0)
    elevation_m = _safe_float(metadata.get('elevation_m'), default=0.0)
    light_extinction_kd = _safe_float(metadata.get('light_extinction_kd'), default=0.0)
    fetch_m = _metadata_value(metadata, 'fetch_m', 'effective_fetch_m', default=0.0)
    wind_exposure_index = _safe_float(metadata.get('wind_exposure_index'), default=0.0)
    reservoir_indicator = _metadata_reservoir_indicator(metadata)
    if not np.isfinite(reservoir_indicator):
    ...
    heat_capacity_areal_norm = normalize_positive_reference(
        mean_depth_m,
        PINN_MAX_MEAN_DEPTH_REFERENCE_M,
    )
    light_penetration_ratio = _metadata_value(
        metadata,
        'light_penetration_ratio',
        'z_light_to_depth_ratio',
        'light_penetration_to_depth_ratio',
        default=np.nan,
    )
    if not np.isfinite(light_penetration_ratio):
        if np.isfinite(light_extinction_kd) and light_extinction_kd > 0.0 and max_depth_m > 0.0:
            light_penetration_ratio = (1.0 / light_extinction_kd) / max_depth_m
        else:
            light_penetration_ratio = 0.0
    hydrology_missing_flag = _metadata_value(
        metadata,
        'hydrology_missing_flag',
        'reservoir_missing_hydrology',
        default=1.0 if (
            reservoir_indicator > 0.0
            and not (np.isfinite(discharge_m3_s) and discharge_m3_s > 0.0)
            and not (np.isfinite(residence_time_days) and residence_time_days > 0.0)
        ) else 0.0,
    )
    kd_source_type_code = _metadata_value(
        metadata,
        'kd_source_type_code',
        'Kd_source_type_code',
        'kd_source_type_norm',
        default=1.0 if light_extinction_kd > 0.0 else 0.0,
    )
    basin_shape_factor = _safe_float(metadata.get('basin_shape_factor'), default=np.nan)
    ...
    return {
        ...
        'heat_capacity_areal_norm': heat_capacity_areal_norm,
        'light_penetration_ratio': float(np.clip(light_penetration_ratio, 0.0, 2.0)),
        'hydrology_missing_flag': normalize_unit_interval(hydrology_missing_flag),
        'kd_source_type_norm': normalize_unit_interval(kd_source_type_code),
    }
