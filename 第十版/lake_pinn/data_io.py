"""Data loading, gap filling, profile splitting, and training batch builders."""
...
from .constants import (
    DEFAULT_INITIAL_WATER_TEMPERATURE_C,
    DEFAULT_PROFILE_SPLIT_PATTERN,
...
from .physics import saturation_vapor_pressure_np
...
def normalize_task_mode(mode: str | None) -> str:
    """Normalize high-level task semantics.

    LakePINN now supports only offline multi-lake reconstruction. Forecast and
    nowcast task modes were removed to keep the training/evaluation semantics
    aligned with global reconstruction experiments.
    """
    mode = str(mode or 'analysis').strip().lower()
    aliases = {
    ...
    if mode != 'analysis':
        raise ValueError(
            f"Unsupported task_mode={mode!r}; forecast/nowcast modes were removed. "
            "Use analysis/reconstruction."
        )
    return mode


def data_fill_mode_for_task_mode(task_mode: str | None) -> str:
    """Return gap-filling semantics implied by the task mode."""
...
def normalize_data_fill_mode(mode: str | None) -> str:
    ...
    if mode != 'reconstruction':
        raise ValueError(
            f"Unsupported data_fill_mode={mode!r}; forecast fill mode was removed. "
            "Use reconstruction."
        )
    return mode
...
def fill_numeric_series(
    ...
    """Fill a numeric daily series with offline reconstruction semantics."""
    filled = pd.to_numeric(values, errors='coerce')
    normalize_data_fill_mode(data_fill_mode)
...
def prepare_lst_daily_frame(lst: pd.DataFrame) -> pd.DataFrame:
    ...
    ice_values = pick_numeric_series_ci(
        lst,
        ['ice_fraction', 'ice_cover_fraction', 'lake_ice_fraction', 'ice_cover', 'ice_concentration'],
    )
    if not ice_values.isna().all():
        if ice_values.max(skipna=True) > 1.5:
            ice_values = ice_values / 100.0
        out['ice_fraction'] = ice_values.clip(lower=0.0, upper=1.0)
        aggregations['ice_fraction'] = 'mean'
...
def add_past_forcing_memory_features(
    frame: pd.DataFrame,
    ...
    *,
    ...
    def history_series(series: pd.Series) -> pd.Series:
        source = series if include_current_day else series.shift(1)
        return source.fillna(series)
    ...
    def past_sum_series(series: pd.Series, window: int) -> pd.Series:
        return history_series(series).rolling(window=window, min_periods=1).sum()

    def past_mean_series(series: pd.Series, window: int) -> pd.Series:
        return history_series(series).rolling(window=window, min_periods=1).mean()
    ...
    net_radiation_proxy = (
        pd.to_numeric(out['Solar_W_m2'], errors='coerce').fillna(0.0)
        + pd.to_numeric(out['Longwave_W_m2'], errors='coerce').fillna(0.0)
        - pd.to_numeric(out['latent_heat_upward_W_m2'], errors='coerce').fillna(0.0)
        - pd.to_numeric(out['sensible_heat_upward_W_m2'], errors='coerce').fillna(0.0)
    )
    out['net_radiation_7d'] = past_sum_series(net_radiation_proxy, 7)
    out['net_radiation_30d'] = past_sum_series(net_radiation_proxy, 30)
    out['wind_mean_7d'] = past_mean('wind_speed_m_per_s', 7)
    wind_energy = pd.to_numeric(out['wind_speed_m_per_s'], errors='coerce').fillna(0.0).clip(lower=0.0).pow(3)
    out['wind_energy_7d'] = past_mean_series(wind_energy, 7)
    out['wind_energy_30d'] = past_mean_series(wind_energy, 30)
    max_depth_value = float(max_depth_m) if max_depth_m is not None else np.nan
    if not np.isfinite(max_depth_value) or max_depth_value <= 0.0:
        max_depth_value = 20.0
    if 'effective_fetch' in out.columns:
        fetch_series = pd.to_numeric(out['effective_fetch'], errors='coerce')
    else:
        fetch_series = pd.Series(0.0, index=out.index, dtype=np.float32)
    fetch_norm = fetch_series.fillna(0.0).clip(lower=0.0) / float(PINN_FETCH_REFERENCE_M)
    depth_norm = max(max_depth_value / float(PINN_MAX_MEAN_DEPTH_REFERENCE_M), 1.0e-3)
    out['wind_mixing_potential_7d'] = out['wind_energy_7d'] * fetch_norm / depth_norm
    out['wind_mixing_potential_30d'] = out['wind_energy_30d'] * fetch_norm / depth_norm
    out['lst_mean_7d'] = past_mean('LST_surface_C', 7)
    raw_lswt = (
        pd.to_numeric(out['LSWT_open_water_C'], errors='coerce')
        if 'LSWT_open_water_C' in out.columns
        else pd.Series(np.nan, index=out.index, dtype=np.float32)
    )
    raw_available = raw_lswt.notna()
    raw_available_history = raw_available if include_current_day else raw_available.shift(1, fill_value=False)
    raw_value_history = raw_lswt if include_current_day else raw_lswt.shift(1)
    days_since = []
    days_without_raw = float(PINN_LSWT_MEMORY_DAYS_REFERENCE)
    for observed in raw_available_history.astype(bool).tolist():
        if observed:
            days_without_raw = 0.0
        else:
            days_without_raw += 1.0
        days_since.append(days_without_raw)
    out['days_since_last_raw_LSWT'] = np.asarray(days_since, dtype=np.float32)
    out['raw_LSWT_valid_count_30d'] = (
        raw_available_history.astype(np.float32).rolling(window=30, min_periods=1).sum()
    )
    raw_ffill = raw_value_history.ffill()
    out['raw_LSWT_trend_7d'] = ((raw_ffill - raw_ffill.shift(7)) / 7.0).fillna(0.0)
    heating_degree = (history_source('T_air_C') - 4.0).clip(lower=0.0)
    cooling_degree = (4.0 - history_source('T_air_C')).clip(lower=0.0)
    out['heating_degree_days_30d'] = heating_degree.rolling(window=30, min_periods=1).sum()
    out['cooling_degree_days_30d'] = cooling_degree.rolling(window=30, min_periods=1).sum()
    return out
...
def load_training_frame(
    ...
    merged['LST_observed_raw_K'] = pd.to_numeric(merged['LST_surface_K'], errors='coerce')
    merged['LST_observed_raw_C'] = merged['LST_observed_raw_K'] - 273.15
    ...
    merged['LST_filled_C'] = merged['LST_surface_C']
    # Backward-compatible alias: raw means direct satellite retrieval, not
    # pipeline-filled LST. Strong LST labels should use this observed field.
    merged['LST_raw_C'] = merged['LST_observed_raw_C']
    ...
    # Prefer explicit shortwave flux (W/m2) when available.  Some ERA5 lake
    # products include Is_J_per_m2, but that is not the same as downwelling
    # shortwave radiation and can be orders of magnitude smaller.
    solar_w_direct = pick_numeric_series(
        merged,
        ['Solar_W_m2', 'ssrd_W_per_m2', 'shortwave_W_m2', 'shortwave', 'downwelling_shortwave_W_m2'],
    )
    solar_j_fallback = pick_numeric_series(merged, ['solar_J_m2', 'ssrd_J_per_m2'])
    if not solar_w_direct.isna().all():
        merged['Solar_W_m2'] = solar_w_direct
        merged['Solar_J_m2'] = merged['Solar_W_m2'] * SECONDS_PER_DAY
    else:
        merged['Solar_J_m2'] = solar_j_fallback
    ...
    raw_ice_fraction = pick_numeric_series(
        merged,
        ['ice_fraction', 'ice_cover_fraction', 'lake_ice_fraction', 'ice_cover', 'ice_concentration'],
    )
    if not raw_ice_fraction.isna().all() and raw_ice_fraction.max(skipna=True) > 1.5:
        raw_ice_fraction = raw_ice_fraction / 100.0
    raw_snow_depth = pick_numeric_series(
        merged,
        ['snow_depth_m', 'SnowDepth_m', 'snow_depth', 'sd_m', 'snow_water_equivalent_m'],
    )
    raw_ice_thickness = pick_numeric_series(
        merged,
    ...
    merged['ice_fraction'] = filled_optional_series(
        raw_ice_fraction,
        default=0.0,
        lower=0.0,
        upper=1.0,
    )
    merged['ice_fraction_observed'] = (
        pd.to_numeric(raw_ice_fraction, errors='coerce').notna().astype(np.float32)
    )
    max_depth_for_prior = metadata.get('max_depth_m', np.nan)
    thermal_regime = infer_thermal_regime(metadata, merged)
    bottom_prior_c = infer_bottom_temp_prior_c(metadata, merged, max_depth=max_depth_for_prior)
    metadata['thermal_regime'] = thermal_regime
    metadata['bottom_temp_prior_c'] = float(bottom_prior_c)
    merged['thermal_regime'] = thermal_regime
    merged['BottomTemp_prior_C'] = float(bottom_prior_c)
    merged['ice_risk_prior'] = infer_ice_risk_prior(metadata, merged)
    merged['ice_fraction'] = np.maximum(
        pd.to_numeric(merged['ice_fraction'], errors='coerce').fillna(0.0).to_numpy(dtype=np.float32),
        pd.to_numeric(merged['ice_risk_prior'], errors='coerce').fillna(0.0).to_numpy(dtype=np.float32),
    )
    ...
    if apply_freezing_lst_rule:
        is_freezing = (
            ...
            & (pd.to_numeric(merged['ice_risk_prior'], errors='coerce').fillna(0.0) >= 0.5)
            & (pd.to_numeric(merged['T_air_C'], errors='coerce') <= 0.5)
        )
        merged.loc[is_freezing, 'LST_surface_C'] = 0.0
        merged.loc[is_freezing, 'LST_filled_C'] = 0.0
        merged.loc[is_freezing, 'LST_imputed_by_freezing_rule'] = 1.0

    original_bottom_temp = pd.to_numeric(merged['BottomTemp_C'], errors='coerce')
    bottom_unreasonable = (original_bottom_temp < 0.5) | (original_bottom_temp > 10.0)
    cold_ice_context = (
        (thermal_regime == 'cold_ice_prone')
        & (pd.to_numeric(merged['ice_risk_prior'], errors='coerce').fillna(0.0) >= 0.5)
    )
    merged['BottomTemp_imputed_by_prior_rule'] = 0.0
    merged['BottomTemp_imputed_by_4C_rule'] = 0.0
    if apply_bottom_4c_rule:
        bottom_4c_mask = cold_ice_context & bottom_unreasonable
        merged.loc[bottom_4c_mask, 'BottomTemp_C'] = 4.0
        merged.loc[bottom_4c_mask, 'BottomTemp_imputed_by_prior_rule'] = 1.0
        merged.loc[bottom_4c_mask, 'BottomTemp_imputed_by_4C_rule'] = 1.0
    bottom_after_rules = pd.to_numeric(merged['BottomTemp_C'], errors='coerce')
    if bottom_after_rules.isna().all():
        bottom_prior_fill_mask = pd.Series(True, index=merged.index)
    else:
        bottom_prior_fill_mask = pd.Series(False, index=merged.index)
    merged.loc[bottom_prior_fill_mask, 'BottomTemp_imputed_by_prior_rule'] = 1.0
    merged['BottomTemp_C'] = fill_numeric_series(
        merged['BottomTemp_C'],
        data_fill_mode,
        default=float(bottom_prior_c),
    )
    ...
    merged['ice_mask'] = ice_mask.astype(np.float32)
    # Observation labels only use direct satellite retrievals. Filled values
    # remain available as model features through LST_surface_C/LST_filled_C.
    merged['LSWT_open_water_C'] = merged['LST_observed_raw_C'].where(~ice_mask)
    merged['IST_snow_ice_C'] = merged['LST_observed_raw_C'].where(ice_mask)
    merged['LST_quality_factor'] = compute_lst_quality_factor(merged)
    merged['snow_depth_m'] = filled_optional_series(
        raw_snow_depth,
        default=0.0,
        lower=0.0,
        upper=5.0,
    )
    if raw_ice_thickness.isna().all():
        ice_thickness_default = 0.05 + 0.45 * pd.to_numeric(merged['ice_fraction'], errors='coerce').fillna(0.0)
        ice_thickness_default = ice_thickness_default.where(pd.to_numeric(merged['ice_fraction'], errors='coerce').fillna(0.0) > 0.0, 0.0)
        raw_ice_thickness = ice_thickness_default
    merged['ice_thickness_m'] = filled_optional_series(
        raw_ice_thickness,
        default=0.0,
        lower=0.0,
        upper=5.0,
    ...
    default_kd = _safe_float(metadata.get('light_extinction_kd'), np.nan)
    ...
    default_fetch_m = _safe_float(metadata.get('fetch_m', metadata.get('effective_fetch_m', 0.0)), 0.0)
    ...
    max_depth_for_features = _safe_float(metadata.get('max_depth_m'), np.nan)
    if not np.isfinite(max_depth_for_features) or max_depth_for_features <= 0.0:
        max_depth_for_features = _safe_float(metadata.get('mean_depth_m'), 20.0)
    mean_depth_for_features = _safe_float(metadata.get('mean_depth_m'), np.nan)
    if not np.isfinite(mean_depth_for_features) or mean_depth_for_features <= 0.0:
        mean_depth_for_features = max_depth_for_features * 0.35
    kd_for_features = pd.to_numeric(merged['light_extinction_kd'], errors='coerce').replace(0.0, np.nan)
    kd_median = float(kd_for_features.median(skipna=True)) if not kd_for_features.dropna().empty else np.nan
    if not np.isfinite(kd_median) or kd_median <= 0.0:
        kd_median = _safe_float(metadata.get('light_extinction_kd'), np.nan)
    z_light_m = 1.0 / kd_median if np.isfinite(kd_median) and kd_median > 0.0 else 0.0
    light_penetration_ratio = (
        z_light_m / max(max_depth_for_features, 1.0e-6)
        if z_light_m > 0.0 else 0.0
    )
    hydrology_missing_flag = float(
        raw_water_level_anomaly.isna().all()
        and raw_water_level.isna().all()
        and raw_net_inflow.isna().all()
        and raw_inflow.isna().all()
        and raw_outflow.isna().all()
        and _safe_float(metadata.get('discharge_m3_s'), 0.0) <= 0.0
    )
    if not raw_light_extinction_kd.isna().all():
        kd_source_type_code = 1.0
    elif pd.to_numeric(merged['Secchi_m'], errors='coerce').fillna(0.0).max() > 0.0:
        kd_source_type_code = 0.66
    elif np.isfinite(default_kd) and default_kd > 0.0:
        kd_source_type_code = 0.33
    else:
        kd_source_type_code = 0.0
    metadata['heat_capacity_areal_j_m2_c'] = float(RHO_CP * mean_depth_for_features)
    metadata['light_penetration_ratio'] = float(np.clip(light_penetration_ratio, 0.0, 2.0))
    metadata['hydrology_missing_flag'] = hydrology_missing_flag
    metadata['kd_source_type_code'] = float(kd_source_type_code)
    merged['heat_capacity_areal'] = metadata['heat_capacity_areal_j_m2_c']
    merged['light_penetration_ratio'] = metadata['light_penetration_ratio']
    merged['hydrology_missing_flag'] = hydrology_missing_flag
    merged['Kd_source_type_code'] = float(kd_source_type_code)
    ...
    merged = add_past_forcing_memory_features(
        ...
        max_depth_m=max_depth_for_features,
    )
    ...
    metadata['reservoir_indicator'] = static_feature_values['reservoir_indicator']
    metadata['residence_time_norm'] = static_feature_values['residence_time_norm']
    metadata['shoreline_length_norm'] = static_feature_values['shoreline_length_norm']
    metadata['shoreline_development_norm'] = static_feature_values['shoreline_development_norm']
    metadata['catchment_area_norm'] = static_feature_values['catchment_area_norm']
    metadata['discharge_norm'] = static_feature_values['discharge_norm']
    metadata['heat_capacity_areal_norm'] = static_feature_values['heat_capacity_areal_norm']
    metadata['light_penetration_ratio_norm'] = static_feature_values['light_penetration_ratio']
    metadata['hydrology_missing_flag_norm'] = static_feature_values['hydrology_missing_flag']
    metadata['kd_source_type_norm'] = static_feature_values['kd_source_type_norm']
...
def downsample_profile_train_dates(train_profile_obs, date_fraction=1.0, random_seed=20260511):
    """Keep a deterministic subset of train dates only; val remains untouched."""
...
def fit_surface_bulk_correction(df, metadata, max_depth, profile_obs_data=None, max_surface_depth=1.0):
    ...
    freeze_surface_mask = (
        (infer_thermal_regime(metadata, df) == 'cold_ice_prone')
        & (pd.to_numeric(ice_risk, errors='coerce').fillna(0.0).to_numpy(dtype=np.float64) >= 0.5)
        & (df['T_air_C'].to_numpy(dtype=np.float64) <= 0.5)
    )
    corrected = np.where(freeze_surface_mask, 0.0, corrected)
...
def build_observation_dataframe(
    ...
    if 'LSWT_open_water_C' in df.columns:
        surface_temperature = pd.to_numeric(df['LSWT_open_water_C'], errors='coerce')
    else:
        surface_temperature = pd.Series(np.nan, index=df.index, dtype=np.float32)
    surface_valid = surface_temperature.notna()
    if 'LST_observed_flag' in df.columns:
        observed_flag = pd.to_numeric(df['LST_observed_flag'], errors='coerce').fillna(0.0)
        surface_valid &= observed_flag > 0.5
    elif 'LST_is_filled' in df.columns:
        is_filled = pd.to_numeric(df['LST_is_filled'], errors='coerce').fillna(1.0)
        surface_valid &= is_filled <= 0.5
    if 'LST_quality_factor' in df.columns:
        quality = pd.to_numeric(df['LST_quality_factor'], errors='coerce').fillna(0.0)
        surface_valid &= quality > 0.0
    if 'ice_mask' in df.columns:
        ice_mask = pd.to_numeric(df['ice_mask'], errors='coerce').fillna(0.0)
        surface_valid &= ice_mask <= 0.0
    base_surface = pd.DataFrame(
        {
            'Date': df.loc[surface_valid, 'Date'],
            'Depth_m': float(np.clip(surface_obs_depth_m, 0.0, max_depth)),
            'Temperature_C': surface_temperature.loc[surface_valid],
            'time_norm': df.loc[surface_valid, 'time_norm'],
            'source': 'surface',
        }
    )
    ...
    forcing_columns = [
        ...
        'BottomTemp_prior_C',
        'BottomTemp_imputed_by_prior_rule',
    ...
    if bottom_mask.any() and (
        'BottomTemp_imputed_by_prior_rule' in observations.columns
        or 'BottomTemp_imputed_by_4C_rule' in observations.columns
    ):
        prior_imputed = pd.Series(0.0, index=observations.index, dtype=np.float64)
        if 'BottomTemp_imputed_by_prior_rule' in observations.columns:
            prior_imputed = (
                pd.to_numeric(observations['BottomTemp_imputed_by_prior_rule'], errors='coerce')
                .fillna(0.0)
                .astype(float)
            )
        hard_4c_imputed = pd.Series(0.0, index=observations.index, dtype=np.float64)
        if 'BottomTemp_imputed_by_4C_rule' in observations.columns:
            hard_4c_imputed = (
        ...
        bottom_factor.loc[prior_imputed > 0.5] = 0.25
        bottom_factor.loc[hard_4c_imputed > 0.5] = 0.10
...
def build_initial_condition_profile(df, max_depth, n_points=64, mode='surface_to_bottom_prior'):
    ...
    bottom_prior_series = df['BottomTemp_prior_C'] if 'BottomTemp_prior_C' in df.columns else None
    bottom_prior0 = DEFAULT_INITIAL_WATER_TEMPERATURE_C
    if bottom_prior_series is not None:
        bottom_prior0 = float(pd.to_numeric(bottom_prior_series, errors='coerce').fillna(bottom_prior0).iloc[0])
    if mode in {'surface_to_bottom_prior', 'conditional_prior'}:
        temp_ic = surf0 + (bottom_prior0 - surf0) * (z_ic / max(max_depth, 1e-6))
    elif mode == 'uniform_4c':
