...
第四版路线延续第三版的条件化 Lake PINN，并将代码整理为模块化包：神经网络输出湖泊温度剖面 `T(z,t)`，物理损失负责约束热扩散、表面能量平衡和边界条件，Kalman 用于预测阶段同化，PPO 用于调度损失权重和部分 Kalman 参数。

## 输入与输出

第四版默认面向 17 维扩展 forcing；复用归档第三版 run9/11 维 checkpoint 或做兼容训练时，需要显式设置 `--model-input-dim 11`。下表是 11 维兼容输入的核心字段。
...
`第四版/lake_pinn/` 是当前推荐继续研究的模块化版本。它承接 run9 后续实验，把训练、预测、Kalman 同化、PPO 调度和评分工具拆开维护。
