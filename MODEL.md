...
第三版路线是一个条件化 Lake PINN：神经网络输出湖泊温度剖面 `T(z,t)`，物理损失负责约束热扩散、表面能量平衡和边界条件，Kalman 用于预测阶段同化，PPO 用于调度损失权重和部分 Kalman 参数。
...
`第三版/PPO策略调控_11维主线_20260426.py` 是当前推荐继续研究的版本。它不是所有实验中 RMSE 最低的版本，但在 11 维输入、物理形态和季节过程之间更平衡。

`第三版/PPO策略调控_热收支A线_20260428.py` 是实验线。A3/A4 说明能量版热收支有潜力，但预测阶段仍需继续检查 PPO / Kalman / heat-budget 的耦合。
