...
python lake_profile_scorecard.py ^
  ...
  --label "predict_a" ^
  --pred "outputs\\predict_b\\mohonk_lake_2017_kalman_temperature_depth_predictions.csv" ^
  --label "predict_b_kalman" ^
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-physical-temp-c", type=float, default=-0.5)
...
def compute_heatmap_physics_diagnostics(
    ...
    surface_band_hat = np.nanmean(hat[:, surface_band_mask], axis=1)

    surface_jump = np.abs(np.diff(surface_hat))
    surface_band_jump = np.abs(np.diff(surface_band_hat))
    column_jump = np.nanmax(np.abs(np.diff(hat, axis=0)), axis=1)

    dz = np.diff(depths)
    if len(dz):
        vertical_grad = np.abs(np.diff(hat, axis=1) / dz.reshape(1, -1))
        grad_p995 = float(np.nanpercentile(vertical_grad, 99.5))
        grad_extreme = float(np.nanmax(vertical_grad))
    else:
        grad_p995 = np.nan
        grad_extreme = np.nan
    ...
    unstable = rho_drop > density_inversion_drop_kgm3
    unstable_frac = np.nanmean(unstable, axis=1) if unstable.size else np.zeros(len(dates), dtype=np.float64)

    april_mask = np.asarray(dates.month == 4)
    if np.any(april_mask):
        april_surface_max_hat = float(np.nanmax(surface_hat[april_mask]))
        april_surface_max_obs = float(np.nanmax(surface_obs[april_mask]))
        april_surface_error = april_surface_max_hat - april_surface_max_obs
        april_jump_mask = april_mask[:-1] & april_mask[1:]
        april_surface_jump = float(np.nanmax(surface_jump[april_jump_mask])) if np.any(april_jump_mask) else np.nan
    else:
    ...
    max_surface_jump_idx = int(np.nanargmax(surface_jump)) if surface_jump.size else -1
    max_column_jump_idx = int(np.nanargmax(column_jump)) if column_jump.size else -1

    return {
        "heatmap_min_temp_c": float(np.nanmin(hat)),
        "heatmap_max_temp_c": float(np.nanmax(hat)),
        "heatmap_max_surface_jump_c_day": float(np.nanmax(surface_jump)) if surface_jump.size else np.nan,
        "heatmap_max_surface_jump_date": dates[max_surface_jump_idx + 1] if max_surface_jump_idx >= 0 else pd.NaT,
        "heatmap_max_surface_band_jump_c_day": float(np.nanmax(surface_band_jump)) if surface_band_jump.size else np.nan,
        "heatmap_max_column_jump_c_day": float(np.nanmax(column_jump)) if column_jump.size else np.nan,
        ...
        "heatmap_max_density_unstable_layer_frac": float(np.nanmax(unstable_frac)) if unstable_frac.size else np.nan,
        "heatmap_density_unstable_days": int(np.nansum(unstable_frac > 0.20)) if unstable_frac.size else 0,
...
def evaluate_vetoes(
    ...
    temperature_range_pass = bool(
        heatmap_metrics["heatmap_min_temp_c"] >= args.min_physical_temp_c
        and heatmap_metrics["heatmap_max_temp_c"] <= args.max_physical_temp_c
    )
    surface_jump_pass = bool(heatmap_metrics["heatmap_max_surface_jump_c_day"] <= args.max_surface_jump_c_per_day)
    surface_band_jump_pass = bool(
        heatmap_metrics["heatmap_max_surface_band_jump_c_day"] <= args.max_surface_band_jump_c_per_day
    )
    column_jump_pass = bool(heatmap_metrics["heatmap_max_column_jump_c_day"] <= args.max_column_jump_c_per_day)
    ...
    april_spike_pass = bool(
        ...
        and heatmap_metrics["april_surface_max_jump_c_day"] <= args.max_april_surface_jump_c_per_day
    )
...
def write_scorecard_report(merged: pd.DataFrame, output_path: Path) -> None:
    """Write a compact PNG scorecard for daily model selection."""
    ...
    for _, row in merged.iterrows():
        ...
        for _, column, formatter in report_cols:
            rendered.append(formatter(row[column]) if column in row else "")
        rows.append(rendered)

    n_rows = max(len(rows), 1)
    fig_width = 18
    fig_height = max(4.2, 1.0 + 0.55 * n_rows)
    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    ax.axis("off")
    ax.set_title(
        "湖泊温度剖面评分 / Lake Profile Scorecard",
        fontsize=18,
        fontweight="bold",
        loc="left",
        pad=18,
    ...
    ax.text(
        0.0,
        0.96,
        "选择规则 / Selection rule: 先过物理底线和热图物理，再看季节过程与数值精度。",
