...
The expected data formats are described in [`DATA.md`](./DATA.md). The current recommended model line is the third-edition 11-feature PINN + PPO/Kalman pipeline described in [`MODEL.md`](./MODEL.md).
...
- The current research mainline: `第三版/PPO策略调控_11维主线_20260426.py`.
...
The project position is summarized in [`EXPERIMENTS.md`](./EXPERIMENTS.md): the historical second-edition route has the lowest Mohonk 2017 RMSE, while the third-edition 11-feature route is the recommended line for continued research.
